...
// jixu modified to supported SI 100M/1000M net test
// @2017.03.28
#if 0
		/* 100M only */
		if (fep->phy_dev) {
			if (fep->phy_dev->speed == SPEED_100) {
				netdev_info(ndev, "Ethernet speed(SPEED_100) has been detected !\n");
			}
			rcntl &= ~(1 << 9);
		}
#else 
...
static int fec_enet_mii_probe(struct net_device *ndev)
{
	...
	netdev_info(ndev, "Freescale FEC PHY driver [%s] (mii_bus:phy_addr=%s, irq=%d, phy_id=%d)\n",
		    fep->phy_dev->drv->name, dev_name(&fep->phy_dev->dev),
		    fep->phy_dev->irq, fep->phy_dev->phy_id);
               ...
               size_t count, loff_t *offset)
{
	...
	for (i = 0 ; i < PHY_MAX_ADDR; i++) {
		if (fep->mii_bus->phy_map[i]) {
			//printk("[SI TEST] fec_main.c:%d fep->mii_bus->phy_map[%d]->drv->name:%s \n", 
			//__LINE__, i, fep->mii_bus->phy_map[i]->drv->name);
			break;
		}
	}
...
// @2017.03.29
#if 0	// RTL8211F
	/* Access RTL8211F internal register to measure IEEE waveform */
	if (strstr(line, "0")) {
		fep->mii_bus->write(fep->mii_bus, i, 0x1f, 0xd04);
		fep->mii_bus->write(fep->mii_bus, i, 0x11, 0x0);
		fep->mii_bus->write(fep->mii_bus, i, 0x1f, 0x0);
		fep->mii_bus->write(fep->mii_bus, i, 0x9, 0x0);
		printk("%s:%d - mode 0 \n", __func__, __LINE__);
	} else if (strstr(line, "D")) {
		fep->mii_bus->write(fep->mii_bus, i, 0x1f, 0xd04);
		fep->mii_bus->write(fep->mii_bus, i, 0x11, 0x0);
		fep->mii_bus->write(fep->mii_bus, i, 0x1f, 0x0);
		fep->mii_bus->write(fep->mii_bus, i, 0x18, 0x2118);		// RGMII between PHY & MAC
		//fep->mii_bus->write(fep->mii_bus, i, 0x18, 0x2198);	// RMII between PHY & MAC
		fep->mii_bus->write(fep->mii_bus, i, 0x9, 0x0200);
		fep->mii_bus->write(fep->mii_bus, i, 0x0, 0x9200);
		printk("%s:%d - mode D \n", __func__, __LINE__);
	} else {
		if (strstr(line, "1")) {
			fep->mii_bus->write(fep->mii_bus, i, 0x1f, 0xd04);
			fep->mii_bus->write(fep->mii_bus, i, 0x11, 0x0002);
			fep->mii_bus->write(fep->mii_bus, i, 0x9, 0x2000);
			printk("%s:%d - mode 1 \n", __func__, __LINE__);
		} else if (strstr(line, "2")) {
			fep->mii_bus->write(fep->mii_bus, i, 0x1f, 0xd04);
			fep->mii_bus->write(fep->mii_bus, i, 0x11, 0x0002);
			fep->mii_bus->write(fep->mii_bus, i, 0x9, 0x4000);	
			printk("%s:%d - mode 2 \n", __func__, __LINE__);
		} else if (strstr(line, "4")) {
			fep->mii_bus->write(fep->mii_bus, i, 0x1f, 0xd04);
			fep->mii_bus->write(fep->mii_bus, i, 0x11, 0x0002);
			fep->mii_bus->write(fep->mii_bus, i, 0x9, 0x8000);
			printk("%s:%d - mode 4 \n", __func__, __LINE__);
		} else if (strstr(line, "A")) {
			fep->mii_bus->write(fep->mii_bus, i, 0x1f, 0xd04);
			fep->mii_bus->write(fep->mii_bus, i, 0x11, 0x0004);
			fep->mii_bus->write(fep->mii_bus, i, 0x1f, 0x0);
			fep->mii_bus->write(fep->mii_bus, i, 0x18, 0x2318);		
			fep->mii_bus->write(fep->mii_bus, i, 0x9, 0x0);
			fep->mii_bus->write(fep->mii_bus, i, 0x0, 0x2100);
			printk("%s:%d - mode A \n", __func__, __LINE__);
		} else if (strstr(line, "B")) {
			fep->mii_bus->write(fep->mii_bus, i, 0x1f, 0xd04);
			fep->mii_bus->write(fep->mii_bus, i, 0x11, 0x0004);
			fep->mii_bus->write(fep->mii_bus, i, 0x1f, 0x0);
			fep->mii_bus->write(fep->mii_bus, i, 0x18, 0x2218);
			fep->mii_bus->write(fep->mii_bus, i, 0x9, 0x0);
			fep->mii_bus->write(fep->mii_bus, i, 0x0, 0x2100);
			printk("%s:%d - mode B \n", __func__, __LINE__);
		} else {
			printk("%s:%d - disable test mode \n", __func__, __LINE__);
	...
	if (strstr(line, "H")) {
		printk("RTL8211E IEEE WAVEFORM TEST API. \n");
		printk("%s:%d - help info. \n", __func__, __LINE__);
		printk("[GET HELP info] echo \"H\" > /proc/net_testmode \n");
		printk("[ENABLE IEEE WAVEFORM TEST] echo \"E\" > /proc/net_testmode \n");
		printk("[1000M MODE1 ENABLE] - echo \"1E\" > /proc/net_testmode \n");
		printk("[1000M MODE1 DISABLE] - echo \"1D\" > /proc/net_testmode \n");
		printk("[1000M MODE4 ENABLE] - echo \"4E\" > /proc/net_testmode \n");
		printk("[1000M MODE4 DISABLE] - echo \"4D\" > /proc/net_testmode \n");
		printk("[100M OUTPUT FROM CHANNEL A] - echo \"OA\" > /proc/net_testmode \n");
		printk("[100M OUTPUT FROM CHANNEL B] - echo \"OB\" > /proc/net_testmode \n");
		printk("[100M OUTPUT DISABLE] - echo \"OD\" > /proc/net_testmode \n");
		printk("[DISABLE IEEE WAVEFORM TEST] - echo \"D\" > /proc/net_testmode \n");
	} else if (strstr(line, "1E")) {
		printk("%s:%d - 1000M MODE1 ENABLE. \n", __func__, __LINE__);
		fep->mii_bus->write(fep->mii_bus, i, 0x1f, 0x0000);
		fep->mii_bus->write(fep->mii_bus, i, 0x9, 0x2000);
	} else if (strstr(line, "1D")) {
		printk("%s:%d - 1000M MODE1 DISABLE. \n", __func__, __LINE__);
		fep->mii_bus->write(fep->mii_bus, i, 0x1f, 0x0000);
		fep->mii_bus->write(fep->mii_bus, i, 0x9, 0x0000);
	} else if (strstr(line, "4E")) {
		printk("%s:%d - 1000M MODE4 ENABLE. \n", __func__, __LINE__);
		fep->mii_bus->write(fep->mii_bus, i, 0x1f, 0x0000);
		fep->mii_bus->write(fep->mii_bus, i, 0x9, 0x8000);
	} else if (strstr(line, "4D")) {
		printk("%s:%d - 1000M MODE4 DISABLE. \n", __func__, __LINE__);
		fep->mii_bus->write(fep->mii_bus, i, 0x1f, 0x0000);
		fep->mii_bus->write(fep->mii_bus, i, 0x9, 0x0000);
	} else if (strstr(line, "OA")) {
		printk("%s:%d - 100M OUTPUT FROM CHANNEL A. \n", __func__, __LINE__);
		fep->mii_bus->write(fep->mii_bus, i, 0x1f, 0x0007);
		fep->mii_bus->write(fep->mii_bus, i, 0x1e, 0x002f);
		fep->mii_bus->write(fep->mii_bus, i, 0x17, 0xd818);
		fep->mii_bus->write(fep->mii_bus, i, 0x1e, 0x002d);
		fep->mii_bus->write(fep->mii_bus, i, 0x18, 0xf060);
		fep->mii_bus->write(fep->mii_bus, i, 0x1f, 0x0);
		fep->mii_bus->write(fep->mii_bus, i, 0x10, 0x00ae);
	} else if (strstr(line, "OB")) {
		printk("%s:%d - 100M OUTPUT FROM CHANNEL B. \n", __func__, __LINE__);
		fep->mii_bus->write(fep->mii_bus, i, 0x1f, 0x0007);
		fep->mii_bus->write(fep->mii_bus, i, 0x1e, 0x002f);
		fep->mii_bus->write(fep->mii_bus, i, 0x17, 0xd818);
		fep->mii_bus->write(fep->mii_bus, i, 0x1e, 0x002d);
		fep->mii_bus->write(fep->mii_bus, i, 0x18, 0xf060);
		fep->mii_bus->write(fep->mii_bus, i, 0x1f, 0x0);
		fep->mii_bus->write(fep->mii_bus, i, 0x10, 0x008e);
	} else if (strstr(line, "OD")) {
		printk("%s:%d - 100M OUTPUT DISABLE. \n", __func__, __LINE__);
		fep->mii_bus->write(fep->mii_bus, i, 0x1f, 0x0007);
		fep->mii_bus->write(fep->mii_bus, i, 0x1e, 0x002f);
		fep->mii_bus->write(fep->mii_bus, i, 0x17, 0xd88f);
		fep->mii_bus->write(fep->mii_bus, i, 0x1e, 0x002d);
		fep->mii_bus->write(fep->mii_bus, i, 0x18, 0xf050);
		fep->mii_bus->write(fep->mii_bus, i, 0x1f, 0x0);
		fep->mii_bus->write(fep->mii_bus, i, 0x10, 0x006e);
	} else if (strstr(line, "E")) {
		printk("%s:%d - enable IEEE waveform. \n", __func__, __LINE__);
		fep->mii_bus->write(fep->mii_bus, i, 0x1f, 0x0005);
		fep->mii_bus->write(fep->mii_bus, i, 0x5, 0x8b86);
		fep->mii_bus->write(fep->mii_bus, i, 0x6, 0xe200);
		fep->mii_bus->write(fep->mii_bus, i, 0x1f, 0x0007);
		fep->mii_bus->write(fep->mii_bus, i, 0x1e, 0x0020);
		fep->mii_bus->write(fep->mii_bus, i, 0x15, 0x0108);
		fep->mii_bus->write(fep->mii_bus, i, 0x1f, 0x0000);
	} else if (strstr(line, "D")) {
		printk("%s:%d - disable all IEEE waveform. \n", __func__, __LINE__);
		fep->mii_bus->write(fep->mii_bus, i, 0x1f, 0x0005);
		fep->mii_bus->write(fep->mii_bus, i, 0x5, 0x8b86);
		fep->mii_bus->write(fep->mii_bus, i, 0x6, 0xe201);
		fep->mii_bus->write(fep->mii_bus, i, 0x1f, 0x0007);
		fep->mii_bus->write(fep->mii_bus, i, 0x1e, 0x0020);
		fep->mii_bus->write(fep->mii_bus, i, 0x15, 0x1108);
		fep->mii_bus->write(fep->mii_bus, i, 0x1f, 0x0000);
	} else {
		printk("%s:%d - unknow cmd! \n", __func__, __LINE__);
	}
#endif // RTL8211E
// modified end
...
// @2017.03.27
	...
	if (proc_entry) {
		...
		netdev_info(ndev, "Create proc node net_testmode succeed !\n");
	} else {
		netdev_info(ndev, "Create proc node net_testmode failed !\n");
	}
#endif	// CONFIG_ARCH_ADVANTECH

// jixu modified to walk through the DQA smartbit test(remove .driver.pm )
// @2017.05.10
#if 0
	netdev_info(ndev, "[Warning] Bypass the PM function !\n");
#endif
