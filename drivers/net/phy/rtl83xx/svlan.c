 ...
 */
rtk_api_ret_t rtk_svlan_init(struct phy_device *phydev)
{
    rtk_uint32 i;
    ...
    if ((retVal = rtl8367c_setAsicSvlanPrioritySel(phydev, SPRISEL_CTAGPRI)) != RT_ERR_OK)
        return retVal;

    /*Drop SVLAN untag frame*/
    if ((retVal = rtl8367c_setAsicSvlanIngressUntag(phydev, UNTAG_DROP)) != RT_ERR_OK)
        return retVal;

    /*Drop SVLAN unmatch frame*/
    if ((retVal = rtl8367c_setAsicSvlanIngressUnmatch(phydev, UNMATCH_DROP)) != RT_ERR_OK)
        return retVal;

    /*Set TPID to 0x88a8*/
    if ((retVal = rtl8367c_setAsicSvlanTpid(phydev, 0x88a8)) != RT_ERR_OK)
        return retVal;

    /*Clean Uplink Port Mask to none*/
    if ((retVal = rtl8367c_setAsicSvlanUplinkPortMask(phydev, 0)) != RT_ERR_OK)
        return retVal;
    ...
    for (i=0; i<= RTL8367C_SVIDXMAX; i++)
    {
        ...
        if ((retVal = rtl8367c_setAsicSvlanMemberConfiguration(phydev, i, &svlanMemConf)) != RT_ERR_OK)
            return retVal;
    }
    ...
    {
        if ((retVal = rtl8367c_setAsicSvlanC2SConf(phydev, i, 0,0,0)) != RT_ERR_OK)
            return retVal;
    }
    ...
    for (i=0; i <= RTL8367C_SP2CMAX ; i++)
    {
        ...
        if ((retVal = rtl8367c_setAsicSvlanSP2CConf(phydev, i, &svlanSP2CConf)) != RT_ERR_OK)
            return retVal;
    }
    ...
    for (i=0 ; i<= RTL8367C_MC2SIDXMAX; i++)
    {
        ...
        if ((retVal = rtl8367c_setAsicSvlanMC2SConf(phydev, i, &svlanMC2SConf)) != RT_ERR_OK)
            return retVal;
    }


    if ((retVal = rtk_svlan_lookupType_set(phydev, SVLAN_LOOKUP_S64MBRCGF)) != RT_ERR_OK)
        return retVal;
 ...
 */
rtk_api_ret_t rtk_svlan_servicePort_add(struct phy_device *phydev, rtk_port_t port)
{
    rtk_api_ret_t retVal;
    rtk_uint32 pmsk;
    ...
    if ((retVal = rtl8367c_getAsicSvlanUplinkPortMask(phydev, &pmsk)) != RT_ERR_OK)
        return retVal;

    pmsk = pmsk | (1<<rtk_switch_port_L2P_get(phydev, port));

    if ((retVal = rtl8367c_setAsicSvlanUplinkPortMask(phydev, pmsk)) != RT_ERR_OK)
        return retVal;
 ...
 *      contain accept SVID in S-tag field.
 */
rtk_api_ret_t rtk_svlan_servicePort_get(struct phy_device *phydev, rtk_portmask_t *pSvlan_portmask)
{
    rtk_api_ret_t retVal;
    ...
    if ((retVal = rtl8367c_getAsicSvlanUplinkPortMask(phydev, &phyMbrPmask)) != RT_ERR_OK)
        return retVal;

    if(rtk_switch_portmask_P2L_get(phydev, phyMbrPmask, pSvlan_portmask) != RT_ERR_OK)
 ...
 */
rtk_api_ret_t rtk_svlan_servicePort_del(struct phy_device *phydev, rtk_port_t port)
{
    rtk_api_ret_t retVal;
    rtk_uint32 pmsk;
 ...
 *      User can set mathced ether type as service provider supported protocol.
 */
rtk_api_ret_t rtk_svlan_tpidEntry_set(struct phy_device *phydev, rtk_svlan_tpid_t svlan_tag_id)
{
    rtk_api_ret_t retVal;
    ...
    if ((retVal = rtl8367c_setAsicSvlanTpid(phydev, svlan_tag_id)) != RT_ERR_OK)
        return retVal;
 ...
 *      contain accept SVID in S-tag field.
 */
rtk_api_ret_t rtk_svlan_tpidEntry_get(struct phy_device *phydev, rtk_svlan_tpid_t *pSvlan_tag_id)
{
    rtk_api_ret_t retVal;
    ...
    if ((retVal = rtl8367c_getAsicSvlanTpid(phydev, pSvlan_tag_id)) != RT_ERR_OK)
        return retVal;
 ...
 *      - REF_PB_PRI.
 */
rtk_api_ret_t rtk_svlan_priorityRef_set(struct phy_device *phydev, rtk_svlan_pri_ref_t ref)
{
    rtk_api_ret_t retVal;
    ...
    if ((retVal = rtl8367c_setAsicSvlanPrioritySel(phydev, ref)) != RT_ERR_OK)
        return retVal;
 ...
 *      - REF_PB_PRI
 */
rtk_api_ret_t rtk_svlan_priorityRef_get(struct phy_device *phydev, rtk_svlan_pri_ref_t *pRef)
{
    rtk_api_ret_t retVal;
    ...
    if ((retVal = rtl8367c_getAsicSvlanPrioritySel(phydev, pRef)) != RT_ERR_OK)
        return retVal;
 ...
 */
rtk_api_ret_t rtk_svlan_memberPortEntry_set(struct phy_device *phydev, rtk_vlan_t svid, rtk_svlan_memberCfg_t *pSvlan_cfg)
{
    rtk_api_ret_t retVal;
    rtk_int32 i;
    ...
    RTK_CHK_PORTMASK_VALID(phydev, &(pSvlan_cfg->memberport));

    RTK_CHK_PORTMASK_VALID(phydev, &(pSvlan_cfg->untagport));
    ...
    {
        if ((retVal = rtk_vlan_get(phydev, svid, &vlanCfg)) != RT_ERR_OK)
            return retVal;

        vlanCfg.mbr = pSvlan_cfg->memberport;
        vlanCfg.untag = pSvlan_cfg->untagport;

        if ((retVal = rtk_vlan_set(phydev, svid, &vlanCfg)) != RT_ERR_OK)
            return retVal;
        ...
        if (empty_idx != 0xFF)
        {
            ...
            if ((retVal = rtl8367c_setAsicSvlanMemberConfiguration(phydev, empty_idx, &svlanMemConf)) != RT_ERR_OK)
                return retVal;
    ...
    {
        /*
        if ((retVal = rtl8367c_getAsicSvlanMemberConfiguration(phydev, i, &svlanMemConf)) != RT_ERR_OK)
            return retVal;
        */
        if (svid == svlan_mbrCfgVid[i] && TRUE == svlan_mbrCfgUsage[i])
        {
            ...
            if(rtk_switch_portmask_L2P_get(phydev, &(pSvlan_cfg->memberport), &phyMbrPmask) != RT_ERR_OK)
                return RT_ERR_FAILED;

            svlanMemConf.vs_member = phyMbrPmask;

            if(rtk_switch_portmask_L2P_get(phydev, &(pSvlan_cfg->untagport), &phyMbrPmask) != RT_ERR_OK)
                return RT_ERR_FAILED;
 ...
 */
rtk_api_ret_t rtk_svlan_memberPortEntry_get(struct phy_device *phydev, rtk_vlan_t svid, rtk_svlan_memberCfg_t *pSvlan_cfg)
{
    rtk_api_ret_t retVal;
    rtk_uint32 i;
    rtl8367c_svlan_memconf_t svlanMemConf;
    ...
    for (i = 0; i<= RTL8367C_SVIDXMAX; i++)
    {
        ...
        if (svid == svlanMemConf.vs_svid)
        {
            ...
            if(rtk_switch_portmask_P2L_get(phydev, svlanMemConf.vs_member,&(pSvlan_cfg->memberport)) != RT_ERR_OK)
                return RT_ERR_FAILED;

            if(rtk_switch_portmask_P2L_get(phydev, svlanMemConf.vs_untag,&(pSvlan_cfg->untagport)) != RT_ERR_OK)
                return RT_ERR_FAILED;
 ...
 *      - rtk_svlan_memberCfg_t->priority is priority of SVLAN member configuration.
 */
rtk_api_ret_t rtk_svlan_memberPortEntry_adv_set(struct phy_device *phydev, rtk_uint32 idx, rtk_svlan_memberCfg_t *pSvlan_cfg)
{
    rtk_api_ret_t retVal;
    rtl8367c_svlan_memconf_t svlanMemConf;
    rtk_uint32 phyMbrPmask;
    ...
    if ((retVal = rtl8367c_setAsicSvlanMemberConfiguration(phydev, idx, &svlanMemConf)) != RT_ERR_OK)
        return retVal;
 ...
 *      to receiving from uplink ports. Other SVID S-tag frame or S-untagged frame will be droped.
 */
rtk_api_ret_t rtk_svlan_memberPortEntry_adv_get(struct phy_device *phydev, rtk_uint32 idx, rtk_svlan_memberCfg_t *pSvlan_cfg)
{
    rtk_api_ret_t retVal;
    rtl8367c_svlan_memconf_t svlanMemConf;
    ...
    if ((retVal = rtl8367c_getAsicSvlanMemberConfiguration(phydev, idx, &svlanMemConf)) != RT_ERR_OK)
        return retVal;
 ...
 */
rtk_api_ret_t rtk_svlan_defaultSvlan_set(struct phy_device *phydev, rtk_port_t port, rtk_vlan_t svid)
{
    rtk_api_ret_t retVal;
    rtk_uint32 i;
    rtl8367c_svlan_memconf_t svlanMemConf;
    ...
    for (i = 0; i < RTL8367C_SVIDXNO; i++)
    {
        ...
        if (svid == svlanMemConf.vs_svid)
        {
            if ((retVal = rtl8367c_setAsicSvlanDefaultVlan(phydev, rtk_switch_port_L2P_get(phydev, port), i)) != RT_ERR_OK)
                return retVal;
 ...
 *      is transmit through uplink port with s-tag field
 */
rtk_api_ret_t rtk_svlan_defaultSvlan_get(struct phy_device *phydev, rtk_port_t port, rtk_vlan_t *pSvid)
{
    rtk_api_ret_t retVal;
    rtk_uint32 idx;
    rtl8367c_svlan_memconf_t svlanMemConf;
    ...
    if ((retVal = rtl8367c_getAsicSvlanDefaultVlan(phydev, rtk_switch_port_L2P_get(phydev, port), &idx)) != RT_ERR_OK)
        return retVal;
 ...
 */
rtk_api_ret_t rtk_svlan_c2s_add(struct phy_device *phydev, rtk_vlan_t vid, rtk_port_t src_port, rtk_vlan_t svid)
{
    ...
    /* Check port Valid */
    RTK_CHK_PORT_VALID(phydev, src_port);

    phyPort = rtk_switch_port_L2P_get(phydev, src_port);
    ...
    {
        if ((retVal = rtl8367c_getAsicSvlanC2SConf(phydev, i, &evid, &pmsk, &c2s_svidx)) != RT_ERR_OK)
                return retVal;
        ...
        if (evid == vid)
        {
            ...
            if(pmsk & (1 << phyPort))
            {
                ...
                else
                {
                    ...
                    if ((retVal = rtl8367c_setAsicSvlanC2SConf(phydev, i, vid, pmsk, c2s_svidx)) != RT_ERR_OK)
                        return retVal;
                }
            }
            else
            {
                ...
                {
                    pmsk = pmsk | (1 << phyPort);
                    if ((retVal = rtl8367c_setAsicSvlanC2SConf(phydev, i, vid, pmsk, svidx)) != RT_ERR_OK)
                        return retVal;
    ...
    {
       if ((retVal = rtl8367c_setAsicSvlanC2SConf(phydev, empty_idx, vid, (1<<phyPort), svidx)) != RT_ERR_OK)
           return retVal;
 ...
 */
rtk_api_ret_t rtk_svlan_c2s_del(struct phy_device *phydev, rtk_vlan_t vid, rtk_port_t src_port)
{
    rtk_api_ret_t retVal;
    rtk_uint32 i;
    ...
    for (i = 0; i <= RTL8367C_C2SIDXMAX; i++)
    {
        if ((retVal = rtl8367c_getAsicSvlanC2SConf(phydev, i, &evid, &pmsk, &svidx)) != RT_ERR_OK)
            return retVal;
 ...
 */
rtk_api_ret_t rtk_svlan_c2s_get(struct phy_device *phydev, rtk_vlan_t vid, rtk_port_t src_port, rtk_vlan_t *pSvid)
{
    rtk_api_ret_t retVal;
    rtk_uint32 i;
    rtk_uint32 evid, pmsk, svidx;
    ...
    for (i = 0; i <= RTL8367C_C2SIDXMAX; i++)
    {
        ...
        if (evid == vid)
        {
            if(pmsk & (1 << phyPort))
            {
                if ((retVal = rtl8367c_getAsicSvlanMemberConfiguration(phydev, svidx, &svlanMemConf)) != RT_ERR_OK)
                    return retVal;
 ...
 */
rtk_api_ret_t rtk_svlan_untag_action_set(struct phy_device *phydev, rtk_svlan_untag_action_t action, rtk_vlan_t svid)
{
    rtk_api_ret_t   retVal;
    rtk_uint32      i;
    rtl8367c_svlan_memconf_t svlanMemConf;
    ...
    if ((retVal = rtl8367c_setAsicSvlanIngressUntag(phydev, (rtk_uint32)action)) != RT_ERR_OK)
        return retVal;
    ...
    if(action == UNTAG_ASSIGN)
    {
        for (i = 0; i < RTL8367C_SVIDXNO; i++)
        {
            ...
            if (svid == svlanMemConf.vs_svid)
            {
                if ((retVal = rtl8367c_setAsicSvlanUntagVlan(phydev, i)) != RT_ERR_OK)
                    return retVal;
 ...
 */
rtk_api_ret_t rtk_svlan_untag_action_get(struct phy_device *phydev, rtk_svlan_untag_action_t *pAction, rtk_vlan_t *pSvid)
{
    rtk_api_ret_t   retVal;
    rtk_uint32      svidx;
    rtl8367c_svlan_memconf_t svlanMemConf;
    ...
    if ((retVal = rtl8367c_getAsicSvlanIngressUntag(phydev, pAction)) != RT_ERR_OK)
        return retVal;

    if(*pAction == UNTAG_ASSIGN)
    {
        if ((retVal = rtl8367c_getAsicSvlanUntagVlan(phydev, &svidx)) != RT_ERR_OK)
            return retVal;
 ...
 */
rtk_api_ret_t rtk_svlan_unmatch_action_set(struct phy_device *phydev, rtk_svlan_unmatch_action_t action, rtk_vlan_t svid)
{
    rtk_api_ret_t   retVal;
    rtk_uint32      i;
    rtl8367c_svlan_memconf_t svlanMemConf;
    ...
    if ((retVal = rtl8367c_setAsicSvlanIngressUnmatch(phydev, (rtk_uint32)action)) != RT_ERR_OK)
        return retVal;
    ...
    if(action == UNMATCH_ASSIGN)
    {
        for (i = 0; i < RTL8367C_SVIDXNO; i++)
        {
            ...
            if (svid == svlanMemConf.vs_svid)
            {
                if ((retVal = rtl8367c_setAsicSvlanUnmatchVlan(phydev, i)) != RT_ERR_OK)
                    return retVal;
 ...
 */
rtk_api_ret_t rtk_svlan_unmatch_action_get(struct phy_device *phydev, rtk_svlan_unmatch_action_t *pAction, rtk_vlan_t *pSvid)
{
    rtk_api_ret_t   retVal;
    rtk_uint32      svidx;
    rtl8367c_svlan_memconf_t svlanMemConf;
    ...
    if ((retVal = rtl8367c_getAsicSvlanIngressUnmatch(phydev, pAction)) != RT_ERR_OK)
        return retVal;

    if(*pAction == UNMATCH_ASSIGN)
    {
        if ((retVal = rtl8367c_getAsicSvlanUnmatchVlan(phydev, &svidx)) != RT_ERR_OK)
            return retVal;
 ...
 */
rtk_api_ret_t rtk_svlan_unassign_action_set(struct phy_device *phydev, rtk_svlan_unassign_action_t action)
{
    rtk_api_ret_t   retVal;
    ...
    retVal = rtl8367c_setAsicSvlanEgressUnassign(phydev, (rtk_uint32)action);
 ...
 *      None
 */
rtk_api_ret_t rtk_svlan_unassign_action_get(struct phy_device *phydev, rtk_svlan_unassign_action_t *pAction)
{
    rtk_api_ret_t   retVal;
    ...
    retVal = rtl8367c_getAsicSvlanEgressUnassign(phydev, pAction);
 ...
 */
rtk_api_ret_t rtk_svlan_dmac_vidsel_set(struct phy_device *phydev, rtk_port_t port, rtk_enable_t enable)
{
    rtk_api_ret_t   retVal;
    ...
    if ((retVal = rtl8367c_setAsicSvlanDmacCvidSel(phydev, rtk_switch_port_L2P_get(phydev, port), enable)) != RT_ERR_OK)
            return retVal;
 ...
 */
rtk_api_ret_t rtk_svlan_dmac_vidsel_get(struct phy_device *phydev, rtk_port_t port, rtk_enable_t *pEnable)
{
    rtk_api_ret_t   retVal;
    ...
    if ((retVal = rtl8367c_getAsicSvlanDmacCvidSel(phydev, rtk_switch_port_L2P_get(phydev, port), pEnable)) != RT_ERR_OK)
            return retVal;
 ...
 */
rtk_api_ret_t rtk_svlan_ipmc2s_add(struct phy_device *phydev, ipaddr_t ipmc, ipaddr_t ipmcMsk,rtk_vlan_t svid)
{
    rtk_api_ret_t retVal, i;
    rtk_uint32 empty_idx;
    rtk_uint32 svidx;
    ...
    {
        if ((retVal = rtl8367c_getAsicSvlanMC2SConf(phydev, i, &svlanMC2SConf)) != RT_ERR_OK)
            return retVal;
    ...
    if (empty_idx!=0xFFFF)
    {
        ...
        svlanMC2SConf.sdata = ipmc;
        svlanMC2SConf.smask = ipmcMsk;
        if ((retVal = rtl8367c_setAsicSvlanMC2SConf(phydev, empty_idx, &svlanMC2SConf)) != RT_ERR_OK)
            return retVal;
        return RT_ERR_OK;
    }
 ...
 */
rtk_api_ret_t rtk_svlan_ipmc2s_del(struct phy_device *phydev, ipaddr_t ipmc, ipaddr_t ipmcMsk)
{
    rtk_api_ret_t retVal;
    rtk_uint32 i;
    rtl8367c_svlan_mc2s_t svlanMC2SConf;
 ...
 */
rtk_api_ret_t rtk_svlan_ipmc2s_get(struct phy_device *phydev, ipaddr_t ipmc, ipaddr_t ipmcMsk, rtk_vlan_t *pSvid)
{
    rtk_api_ret_t retVal;
    rtk_uint32 i;
    rtl8367c_svlan_memconf_t svlanMemConf;
    ...
    for (i = 0; i <= RTL8367C_MC2SIDXMAX; i++)
    {
        ...
        if (TRUE == svlanMC2SConf.valid &&
            ...
            svlanMC2SConf.smask == ipmcMsk)
        {
            if ((retVal = rtl8367c_getAsicSvlanMemberConfiguration(phydev, svlanMC2SConf.svidx, &svlanMemConf)) != RT_ERR_OK)
                return retVal;
            *pSvid = svlanMemConf.vs_svid;
            return RT_ERR_OK;
        }
 ...
 */
rtk_api_ret_t rtk_svlan_l2mc2s_add(struct phy_device *phydev, rtk_mac_t mac, rtk_mac_t macMsk, rtk_vlan_t svid)
{
    rtk_api_ret_t retVal, i;
    rtk_uint32 empty_idx;
 ...
 */
rtk_api_ret_t rtk_svlan_l2mc2s_del(struct phy_device *phydev, rtk_mac_t mac, rtk_mac_t macMsk)
{
    rtk_api_ret_t retVal;
    rtk_uint32 i;
 ...
 */
rtk_api_ret_t rtk_svlan_l2mc2s_get(struct phy_device *phydev, rtk_mac_t mac, rtk_mac_t macMsk, rtk_vlan_t *pSvid)
{
    rtk_api_ret_t retVal;
    rtk_uint32 i;
    rtk_uint32 l2add,l2Mask;
 ...
 */
rtk_api_ret_t rtk_svlan_sp2c_add(struct phy_device *phydev, rtk_vlan_t svid, rtk_port_t dst_port, rtk_vlan_t cvid)
{
    rtk_api_ret_t retVal, i;
    ...
    /* Check port Valid */
    RTK_CHK_PORT_VALID(phydev, dst_port);
    port = rtk_switch_port_L2P_get(phydev, dst_port);
    ...
    {
        if ((retVal = rtl8367c_getAsicSvlanSP2CConf(phydev, i, &svlanSP2CConf)) != RT_ERR_OK)
            return retVal;
    ...
    if (empty_idx!=0xFFFF)
    {
        ...
        if ((retVal = rtl8367c_setAsicSvlanSP2CConf(phydev, empty_idx, &svlanSP2CConf)) != RT_ERR_OK)
            return retVal;
        return RT_ERR_OK;
    }
 ...
 */
rtk_api_ret_t rtk_svlan_sp2c_get(struct phy_device *phydev, rtk_vlan_t svid, rtk_port_t dst_port, rtk_vlan_t *pCvid)
{
    rtk_api_ret_t retVal;
    rtk_uint32 i, svidx;
    rtl8367c_svlan_memconf_t svlanMemConf;
    ...
    /* Check port Valid */
    RTK_CHK_PORT_VALID(phydev, dst_port);
    dst_port = rtk_switch_port_L2P_get(phydev, dst_port);
 ...
 */
rtk_api_ret_t rtk_svlan_sp2c_del(struct phy_device *phydev, rtk_vlan_t svid, rtk_port_t dst_port)
{
    rtk_api_ret_t retVal;
    rtk_uint32 i, svidx;
    rtl8367c_svlan_memconf_t svlanMemConf;
 ...
 *      none
 */
rtk_api_ret_t rtk_svlan_lookupType_set(struct phy_device *phydev, rtk_svlan_lookupType_t type)
{
    rtk_api_ret_t   retVal;
    ...
    retVal = rtl8367c_setAsicSvlanLookupType(phydev, (rtk_uint32)type);
 ...
 *      none
 */
rtk_api_ret_t rtk_svlan_lookupType_get(struct phy_device *phydev, rtk_svlan_lookupType_t *pType)
{
    rtk_api_ret_t   retVal;
    ...
    retVal = rtl8367c_getAsicSvlanLookupType(phydev, pType);
 ...
 *      None
 */
rtk_api_ret_t rtk_svlan_trapPri_set(struct phy_device *phydev, rtk_pri_t priority)
{
    rtk_api_ret_t   retVal;
    ...
    retVal = rtl8367c_setAsicSvlanTrapPriority(phydev, priority);
 ...
 *      None
 */
rtk_api_ret_t rtk_svlan_trapPri_get(struct phy_device *phydev, rtk_pri_t *pPriority)
{
    rtk_api_ret_t   retVal;
    ...
    retVal = rtl8367c_getAsicSvlanTrapPriority(phydev, pPriority);
 ...
 *
 */
rtk_api_ret_t rtk_svlan_checkAndCreateMbr(struct phy_device *phydev, rtk_vlan_t vid, rtk_uint32 *pIndex)
{
    rtk_api_ret_t retVal;
    rtk_uint32 svidx;
    ...
    if((retVal = rtl8367c_setAsicSvlanMemberConfiguration(phydev, empty_idx, &svlan_cfg)) != RT_ERR_OK)
        return retVal;
