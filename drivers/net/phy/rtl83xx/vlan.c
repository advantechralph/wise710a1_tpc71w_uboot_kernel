 ...
 */
rtk_api_ret_t rtk_vlan_init(struct phy_device *phydev)
{
    ...
    for (i = 0; i <= RTL8367C_CVIDXMAX; i++)
    {
        ...
        vlanMC.vbpen = 0;
        vlanMC.vbpri = 0;
        if ((retVal = rtl8367c_setAsicVlanMemberConfig(phydev, i, &vlanMC)) != RT_ERR_OK)
            return retVal;
    }
    ...
    vlan4K.fid_msti = 0;
    if ((retVal = rtl8367c_setAsicVlan4kEntry(phydev, &vlan4K)) != RT_ERR_OK)
        return retVal;
    ...
    vlanMC.fid_msti = 0;
    if ((retVal = rtl8367c_setAsicVlanMemberConfig(phydev, 0, &vlanMC)) != RT_ERR_OK)
            return retVal;

    /* Set all ports PVID to default VLAN and tag-mode to original */
    RTK_SCAN_ALL_PHY_PORTMASK(phydev, i)
    {
        if ((retVal = rtl8367c_setAsicVlanPortBasedVID(phydev, i, 0, 0)) != RT_ERR_OK)
            return retVal;
        if ((retVal = rtl8367c_setAsicVlanEgressTagMode(phydev, i, EG_TAG_MODE_ORI)) != RT_ERR_OK)
            return retVal;
    }
    ...
    RTK_SCAN_ALL_PHY_PORTMASK(phydev, i)
    {
        if ((retVal = rtl8367c_setAsicVlanIngressFilter(phydev, i, ENABLED)) != RT_ERR_OK)
            return retVal;
    }

    /* enable VLAN */
    if ((retVal = rtl8367c_setAsicVlanFilter(phydev, ENABLED)) != RT_ERR_OK)
        return retVal;
 ...
 *
 */
rtk_api_ret_t rtk_vlan_set(struct phy_device *phydev, rtk_vlan_t vid, rtk_vlan_cfg_t *pVlanCfg)
{
    rtk_api_ret_t retVal;
    ...
    RTK_CHK_PORTMASK_VALID(phydev, &(pVlanCfg->mbr));

    if (vid <= RTL8367C_VIDMAX)
    {
        /* Check untag port mask valid */
        RTK_CHK_PORTMASK_VALID(phydev, &(pVlanCfg->untag));
    }
    ...
    /* Meter ID */
    if(pVlanCfg->meteridx > RTK_MAX_METER_ID(phydev))
        return RT_ERR_INPUT;
    ...
    if(rtk_switch_portmask_L2P_get(phydev, &(pVlanCfg->mbr), &phyMbrPmask) != RT_ERR_OK)
        return RT_ERR_FAILED;

    if(rtk_switch_portmask_L2P_get(phydev, &(pVlanCfg->untag), &phyUntagPmask) != RT_ERR_OK)
        return RT_ERR_FAILED;
            ...
            if(vlan_mbrCfgUsage[idx] == MBRCFG_USED_BY_VLAN)
            {
                if(vlan_mbrCfgVid[idx] == vid)
                {
                    ...
                    if(phyMbrPmask == 0x00)
                    {
                        ...
                        if ((retVal = rtl8367c_setAsicVlanMemberConfig(phydev, idx, &vlanMC)) != RT_ERR_OK)
                            return retVal;
    ...
    else
    {
        ...
        if( (phyMbrPmask != 0x00) && (update_evid == 0) && (empty_index != 0xFFFF) )
        {
            ...
            vlanMC.vbpen = pVlanCfg->vbpen;
            vlanMC.vbpri = pVlanCfg->vbpri;
            if ((retVal = rtl8367c_setAsicVlanMemberConfig(phydev, empty_index, &vlanMC)) != RT_ERR_OK)
                return retVal;
 ...
 *
 */
rtk_api_ret_t rtk_vlan_get(struct phy_device *phydev, rtk_vlan_t vid, rtk_vlan_cfg_t *pVlanCfg)
{
    rtk_api_ret_t retVal;
    rtk_uint32 phyMbrPmask;
    rtk_uint32 phyUntagPmask;
    ...
    if (vid <= RTL8367C_VIDMAX)
    {
        ...
        if ((retVal = rtl8367c_getAsicVlan4kEntry(phydev, &vlan4K)) != RT_ERR_OK)
            return retVal;

        phyMbrPmask   = vlan4K.mbr;
        phyUntagPmask = vlan4K.untag;
        if(rtk_switch_portmask_P2L_get(phydev, phyMbrPmask, &(pVlanCfg->mbr)) != RT_ERR_OK)
            return RT_ERR_FAILED;

        if(rtk_switch_portmask_P2L_get(phydev, phyUntagPmask, &(pVlanCfg->untag)) != RT_ERR_OK)
            return RT_ERR_FAILED;
    ...
    else
    {
        for (idx = 0; idx <= RTL8367C_CVIDXMAX; idx++)
        {
            if(vlan_mbrCfgUsage[idx] == MBRCFG_USED_BY_VLAN)
            {
                if(vlan_mbrCfgVid[idx] == vid)
                {
                    if ((retVal = rtl8367c_getAsicVlanMemberConfig(phydev, idx, &vlanMC)) != RT_ERR_OK)
                        return retVal;
 ...
 *
 */
rtk_api_ret_t rtk_vlan_egrFilterEnable_set(struct phy_device *phydev, rtk_enable_t egrFilter)
{
    rtk_api_ret_t retVal;
    ...
    /* enable VLAN */
    if ((retVal = rtl8367c_setAsicVlanFilter(phydev, (rtk_uint32)egrFilter)) != RT_ERR_OK)
        return retVal;
 ...
 *
 */
rtk_api_ret_t rtk_vlan_egrFilterEnable_get(struct phy_device *phydev, rtk_enable_t *pEgrFilter)
{
    rtk_api_ret_t retVal;
    rtk_uint32 state;
    ...
    /* enable VLAN */
    if ((retVal = rtl8367c_getAsicVlanFilter(phydev, &state)) != RT_ERR_OK)
        return retVal;
 ...
 *     Set a VLAN Member Configuration entry by index.
 */
rtk_api_ret_t rtk_vlan_mbrCfg_set(struct phy_device *phydev, rtk_uint32 idx, rtk_vlan_mbrcfg_t *pMbrcfg)
{
    rtk_api_ret_t           retVal;
    rtk_uint32              phyMbrPmask;
    ...
    if(pMbrcfg->meteridx > RTK_MAX_METER_ID(phydev))
    ...
    /* Check port mask valid */
    RTK_CHK_PORTMASK_VALID(phydev, &(pMbrcfg->mbr));
    ...
    if(rtk_switch_portmask_L2P_get(phydev, &(pMbrcfg->mbr), &phyMbrPmask) != RT_ERR_OK)
        return RT_ERR_FAILED;

    mbrCfg.mbr = phyMbrPmask;

    if ((retVal = rtl8367c_setAsicVlanMemberConfig(phydev, idx, &mbrCfg)) != RT_ERR_OK)
        return retVal;
 ...
 *     Get a VLAN Member Configuration entry by index.
 */
rtk_api_ret_t rtk_vlan_mbrCfg_get(struct phy_device *phydev, rtk_uint32 idx, rtk_vlan_mbrcfg_t *pMbrcfg)
{
    rtk_api_ret_t           retVal;
    rtk_uint32              phyMbrPmask;
    rtl8367c_vlanconfiguser mbrCfg;
    ...
    if ((retVal = rtl8367c_getAsicVlanMemberConfig(phydev, idx, &mbrCfg)) != RT_ERR_OK)
        return retVal;
    ...
    if(rtk_switch_portmask_P2L_get(phydev, phyMbrPmask, &(pMbrcfg->mbr)) != RT_ERR_OK)
        return RT_ERR_FAILED;
 ...
 */
rtk_api_ret_t rtk_vlan_portPvid_set(struct phy_device *phydev, rtk_port_t port, rtk_vlan_t pvid, rtk_pri_t priority)
{
    rtk_api_ret_t retVal;
    rtk_uint32 index;
    ...
    if((retVal = rtk_vlan_checkAndCreateMbr(phydev, pvid, &index)) != RT_ERR_OK)
        return retVal;

    if ((retVal = rtl8367c_setAsicVlanPortBasedVID(phydev, rtk_switch_port_L2P_get(phydev, port), index, priority)) != RT_ERR_OK)
        return retVal;
 ...
 */
rtk_api_ret_t rtk_vlan_portPvid_get(struct phy_device *phydev, rtk_port_t port, rtk_vlan_t *pPvid, rtk_pri_t *pPriority)
{
    rtk_api_ret_t retVal;
    ...
    if ((retVal = rtl8367c_getAsicVlanPortBasedVID(phydev, rtk_switch_port_L2P_get(phydev, port), &index, &pri)) != RT_ERR_OK)
        return retVal;

    memset(&mbrCfg, 0x00, sizeof(rtl8367c_vlanconfiguser));
    if ((retVal = rtl8367c_getAsicVlanMemberConfig(phydev, index, &mbrCfg)) != RT_ERR_OK)
        return retVal;
 ...
 */
rtk_api_ret_t rtk_vlan_portIgrFilterEnable_set(struct phy_device *phydev, rtk_port_t port, rtk_enable_t igr_filter)
{
    rtk_api_ret_t retVal;
    ...
    if ((retVal = rtl8367c_setAsicVlanIngressFilter(phydev, rtk_switch_port_L2P_get(phydev, port), igr_filter)) != RT_ERR_OK)
        return retVal;
 ...
 *     - ENABLED
 */
rtk_api_ret_t rtk_vlan_portIgrFilterEnable_get(struct phy_device *phydev, rtk_port_t port, rtk_enable_t *pIgr_filter)
{
    rtk_api_ret_t retVal;
    ...
    if ((retVal = rtl8367c_getAsicVlanIngressFilter(phydev, rtk_switch_port_L2P_get(phydev, port), pIgr_filter)) != RT_ERR_OK)
        return retVal;
 ...
 */
rtk_api_ret_t rtk_vlan_portAcceptFrameType_set(struct phy_device *phydev, rtk_port_t port, rtk_vlan_acceptFrameType_t accept_frame_type)
{
    rtk_api_ret_t retVal;
    ...
    if ((retVal = rtl8367c_setAsicVlanAccpetFrameType(phydev, rtk_switch_port_L2P_get(phydev, port), (rtl8367c_accframetype)accept_frame_type)) != RT_ERR_OK)
        return retVal;
 ...
 *     - ACCEPT_FRAME_TYPE_UNTAG_ONLY
 */
rtk_api_ret_t rtk_vlan_portAcceptFrameType_get(struct phy_device *phydev, rtk_port_t port, rtk_vlan_acceptFrameType_t *pAccept_frame_type)
{
    rtk_api_ret_t retVal;
    ...
    if ((retVal = rtl8367c_getAsicVlanAccpetFrameType(phydev, rtk_switch_port_L2P_get(phydev, port), &acc_frm_type)) != RT_ERR_OK)
        return retVal;
 ...
 *      - FRAME_TYPE_LLCOTHER
 */
rtk_api_ret_t rtk_vlan_protoAndPortBasedVlan_add(struct phy_device *phydev, rtk_port_t port, rtk_vlan_protoAndPortInfo_t *pInfo)
{
    ...
    {
        if ((retVal = rtl8367c_getAsicVlanProtocolBasedGroupData(phydev, i, &ppb_data_cfg)) != RT_ERR_OK)
            return retVal;
    ...
    else if (empty<RTL8367C_PROTOVLAN_GROUPNO)
    {
        ...
        if ((retVal = rtl8367c_setAsicVlanProtocolBasedGroupData(phydev, empty, &ppb_data_cfg)) != RT_ERR_OK)
            return retVal;
        used = empty;
    }
    else
    ...
    if((retVal = rtk_vlan_checkAndCreateMbr(phydev, pInfo->cvid, &index)) != RT_ERR_OK)
        return retVal;

    ppb_vlan_cfg.vlan_idx = index;
    ppb_vlan_cfg.valid = TRUE;
    ppb_vlan_cfg.priority = pInfo->cpri;
    if ((retVal = rtl8367c_setAsicVlanPortAndProtocolBased(phydev, rtk_switch_port_L2P_get(phydev, port), used, &ppb_vlan_cfg)) != RT_ERR_OK)
        return retVal;
 ...
 *      - FRAME_TYPE_LLCOTHER
 */
rtk_api_ret_t rtk_vlan_protoAndPortBasedVlan_get(struct phy_device *phydev, rtk_port_t port, rtk_vlan_proto_type_t proto_type, rtk_vlan_protoVlan_frameType_t frame_type, rtk_vlan_protoAndPortInfo_t *pInfo)
{
    rtk_api_ret_t retVal;
    rtk_uint32 i;
    rtk_uint32 ppb_idx;
   ...
   ppb_idx = 0;
    ...
    if ((retVal = rtl8367c_getAsicVlanPortAndProtocolBased(phydev, rtk_switch_port_L2P_get(phydev, port), ppb_idx, &ppb_vlan_cfg)) != RT_ERR_OK)
        return retVal;
 ...
 *      - FRAME_TYPE_LLCOTHER
 */
rtk_api_ret_t rtk_vlan_protoAndPortBasedVlan_del(struct phy_device *phydev, rtk_port_t port, rtk_vlan_proto_type_t proto_type, rtk_vlan_protoVlan_frameType_t frame_type)
{
    rtk_api_ret_t retVal;
    rtk_uint32 i, bUsed;
    rtk_uint32 ppb_idx;
   ...
   ppb_idx = 0;
    ...
    for (i = 0; i<= RTL8367C_PROTOVLAN_GIDX_MAX; i++)
    {
        ...
        if ( (ppb_data_cfg.frameType == (rtl8367c_provlan_frametype)frame_type) && (ppb_data_cfg.etherType == proto_type) )
        {
            ...
            if ((retVal = rtl8367c_setAsicVlanPortAndProtocolBased(phydev, rtk_switch_port_L2P_get(phydev, port), ppb_idx, &ppb_vlan_cfg)) != RT_ERR_OK)
                return retVal;
        }
    }
    ...
    RTK_SCAN_ALL_PHY_PORTMASK(phydev, i)
    {
        if ((retVal = rtl8367c_getAsicVlanPortAndProtocolBased(phydev, i, ppb_idx, &ppb_vlan_cfg)) != RT_ERR_OK)
            return retVal;
    ...
    if (FALSE == bUsed) /*No Port use this PPB Index, Delete it*/
    {
        ...
        if ((retVal = rtl8367c_setAsicVlanProtocolBasedGroupData(phydev, ppb_idx, &ppb_data_cfg)) != RT_ERR_OK)
            return retVal;
    }
 ...
 */
rtk_api_ret_t rtk_vlan_protoAndPortBasedVlan_delAll(struct phy_device *phydev, rtk_port_t port)
{
    rtk_api_ret_t retVal;
    ...
    for (i = 0; i<= RTL8367C_PROTOVLAN_GIDX_MAX; i++)
    {
        ...
        ppb_vlan_cfg.vlan_idx = 0;
        ppb_vlan_cfg.priority = 0;
        if ((retVal = rtl8367c_setAsicVlanPortAndProtocolBased(phydev, rtk_switch_port_L2P_get(phydev, port), i, &ppb_vlan_cfg)) != RT_ERR_OK)
            return retVal;
    }
    ...
    RTK_SCAN_ALL_PHY_PORTMASK(phydev, i)
    {
        ...
        {
            if ((retVal = rtl8367c_getAsicVlanPortAndProtocolBased(phydev, i,j, &ppb_vlan_cfg)) != RT_ERR_OK)
                return retVal;
    ...
    for (i = 0; i<= RTL8367C_PROTOVLAN_GIDX_MAX; i++)
    {
        if (FALSE == bUsed[i]) /*No Port use this PPB Index, Delete it*/
        {
            ppb_data_cfg.etherType=0;
            ppb_data_cfg.frameType=0;
            if ((retVal = rtl8367c_setAsicVlanProtocolBasedGroupData(phydev, i, &ppb_data_cfg)) != RT_ERR_OK)
                return retVal;
        }
    }
 ...
 */
rtk_api_ret_t rtk_vlan_tagMode_set(struct phy_device *phydev, rtk_port_t port, rtk_vlan_tagMode_t tag_mode)
{
    rtk_api_ret_t retVal;
    ...
    if ((retVal = rtl8367c_setAsicVlanEgressTagMode(phydev, rtk_switch_port_L2P_get(phydev, port), tag_mode)) != RT_ERR_OK)
        return retVal;
 ...
 *      - VLAN_TAG_MODE_REAL_KEEP_FORMAT,
 */
rtk_api_ret_t rtk_vlan_tagMode_get(struct phy_device *phydev, rtk_port_t port, rtk_vlan_tagMode_t *pTag_mode)
{
    rtk_api_ret_t retVal;
    ...
    if ((retVal = rtl8367c_getAsicVlanEgressTagMode(phydev, rtk_switch_port_L2P_get(phydev, port), &mode)) != RT_ERR_OK)
        return retVal;
 ...
 *      None.
 */
rtk_api_ret_t rtk_vlan_transparent_set(struct phy_device *phydev, rtk_port_t egr_port, rtk_portmask_t *pIgr_pmask)
{
     rtk_api_ret_t retVal;
     rtk_uint32    pmask;
     ...
     /* Check Port Valid */
     RTK_CHK_PORT_VALID(phydev, egr_port);

     if(NULL == pIgr_pmask)
        return RT_ERR_NULL_POINTER;

     RTK_CHK_PORTMASK_VALID(phydev, pIgr_pmask);

     if(rtk_switch_portmask_L2P_get(phydev, pIgr_pmask, &pmask) != RT_ERR_OK)
        return RT_ERR_FAILED;

     if ((retVal = rtl8367c_setAsicVlanTransparent(phydev, rtk_switch_port_L2P_get(phydev, egr_port), pmask)) != RT_ERR_OK)
         return retVal;
 ...
 *      None.
 */
rtk_api_ret_t rtk_vlan_transparent_get(struct phy_device *phydev, rtk_port_t egr_port, rtk_portmask_t *pIgr_pmask)
{
     rtk_api_ret_t retVal;
     rtk_uint32    pmask;
     ...
     if ((retVal = rtl8367c_getAsicVlanTransparent(phydev, rtk_switch_port_L2P_get(phydev, egr_port), &pmask)) != RT_ERR_OK)
         return retVal;

     if(rtk_switch_portmask_P2L_get(phydev, pmask, pIgr_pmask) != RT_ERR_OK)
        return RT_ERR_FAILED;
 ...
 *      None.
 */
rtk_api_ret_t rtk_vlan_keep_set(struct phy_device *phydev, rtk_port_t egr_port, rtk_portmask_t *pIgr_pmask)
{
     rtk_api_ret_t retVal;
     rtk_uint32    pmask;
     ...
     if ((retVal = rtl8367c_setAsicVlanEgressKeep(phydev, rtk_switch_port_L2P_get(phydev, egr_port), pmask)) != RT_ERR_OK)
         return retVal;
 ...
 *      None.
 */
rtk_api_ret_t rtk_vlan_keep_get(struct phy_device *phydev, rtk_port_t egr_port, rtk_portmask_t *pIgr_pmask)
{
     rtk_api_ret_t retVal;
     rtk_uint32    pmask;
     ...
     if ((retVal = rtl8367c_getAsicVlanEgressKeep(phydev, rtk_switch_port_L2P_get(phydev, egr_port), &pmask)) != RT_ERR_OK)
         return retVal;
 ...
 */
rtk_api_ret_t rtk_vlan_stg_set(struct phy_device *phydev, rtk_vlan_t vid, rtk_stp_msti_id_t stg)
{
    rtk_api_ret_t retVal;
    rtl8367c_user_vlan4kentry vlan4K;
 ...
 */
rtk_api_ret_t rtk_vlan_stg_get(struct phy_device *phydev, rtk_vlan_t vid, rtk_stp_msti_id_t *pStg)
{
    rtk_api_ret_t retVal;
    rtl8367c_user_vlan4kentry vlan4K;
 ...
 */
rtk_api_ret_t rtk_vlan_portFid_set(struct phy_device *phydev, rtk_port_t port, rtk_enable_t enable, rtk_fid_t fid)
{
    rtk_api_ret_t retVal;
    ...
    if ((retVal = rtl8367c_setAsicPortBasedFidEn(phydev, rtk_switch_port_L2P_get(phydev, port), enable))!=RT_ERR_OK)
        return retVal;

    if ((retVal = rtl8367c_setAsicPortBasedFid(phydev, rtk_switch_port_L2P_get(phydev, port), fid))!=RT_ERR_OK)
        return retVal;
 ...
 *      packets will be assigned to the port-based fid regardless vlan tag.
 */
rtk_api_ret_t rtk_vlan_portFid_get(struct phy_device *phydev, rtk_port_t port, rtk_enable_t *pEnable, rtk_fid_t *pFid)
{
    rtk_api_ret_t retVal;
    ...
    if ((retVal = rtl8367c_getAsicPortBasedFidEn(phydev, rtk_switch_port_L2P_get(phydev, port), pEnable))!=RT_ERR_OK)
        return retVal;

    if ((retVal = rtl8367c_getAsicPortBasedFid(phydev, rtk_switch_port_L2P_get(phydev, port), pFid))!=RT_ERR_OK)
        return retVal;
 ...
 *
 */
rtk_api_ret_t rtk_vlan_UntagDscpPriorityEnable_set(struct phy_device *phydev, rtk_enable_t enable)
{
    rtk_api_ret_t retVal;
    ...
    if ((retVal = rtl8367c_setAsicVlanUntagDscpPriorityEn(phydev, (rtk_uint32)enable)) != RT_ERR_OK)
        return retVal;
 ...
 *
 */
rtk_api_ret_t rtk_vlan_UntagDscpPriorityEnable_get(struct phy_device *phydev, rtk_enable_t *pEnable)
{
    rtk_api_ret_t retVal;
    rtk_uint32  value;
    ...
    if ((retVal = rtl8367c_getAsicVlanUntagDscpPriorityEn(phydev, &value)) != RT_ERR_OK)
        return retVal;
 ...
 */
rtk_api_ret_t rtk_stp_mstpState_set(struct phy_device *phydev, rtk_stp_msti_id_t msti, rtk_port_t port, rtk_stp_state_t stp_state)
{
    rtk_api_ret_t retVal;
    ...
    if ((retVal = rtl8367c_setAsicSpanningTreeStatus(phydev, rtk_switch_port_L2P_get(phydev, port), msti, stp_state)) != RT_ERR_OK)
        return retVal;
 ...
 *      - STP_STATE_FORWARDING
 */
rtk_api_ret_t rtk_stp_mstpState_get(struct phy_device *phydev, rtk_stp_msti_id_t msti, rtk_port_t port, rtk_stp_state_t *pStp_state)
{
    rtk_api_ret_t retVal;
    ...
    if ((retVal = rtl8367c_getAsicSpanningTreeStatus(phydev, rtk_switch_port_L2P_get(phydev, port), msti, pStp_state)) != RT_ERR_OK)
        return retVal;
 ...
 *
 */
rtk_api_ret_t rtk_vlan_checkAndCreateMbr(struct phy_device *phydev, rtk_vlan_t vid, rtk_uint32 *pIndex)
{
    rtk_api_ret_t retVal;
    rtl8367c_user_vlan4kentry vlan4K;
    rtl8367c_vlanconfiguser vlanMC;
    ...
    if (vid > RTL8367C_VIDMAX)
    {
        ...
        memset(&vlanMC, 0x00, sizeof(rtl8367c_vlanconfiguser));
        vlanMC.evid = vid;
        if ((retVal = rtl8367c_setAsicVlanMemberConfig(phydev, empty_idx, &vlanMC)) != RT_ERR_OK)
            return retVal;
    }
    else
    {
 ...
 *
 */
rtk_api_ret_t rtk_vlan_reservedVidAction_set(struct phy_device *phydev, rtk_vlan_resVidAction_t action_vid0, rtk_vlan_resVidAction_t action_vid4095)
{
    rtk_api_ret_t retVal;
    ...
    if ((retVal = rtl8367c_setReservedVidAction(phydev, (rtk_uint32)action_vid0, (rtk_uint32)action_vid4095)) != RT_ERR_OK)
        return retVal;
 ...
 *
 */
rtk_api_ret_t rtk_vlan_reservedVidAction_get(struct phy_device *phydev, rtk_vlan_resVidAction_t *pAction_vid0, rtk_vlan_resVidAction_t *pAction_vid4095)
{
    rtk_api_ret_t retVal;
    ...
    if ((retVal = rtl8367c_getReservedVidAction(phydev, (rtk_uint32 *)pAction_vid0, (rtk_uint32 *)pAction_vid4095)) != RT_ERR_OK)
        return retVal;
 ...
 *
 */
rtk_api_ret_t rtk_vlan_realKeepRemarkEnable_set(struct phy_device *phydev, rtk_enable_t enabled)
{
    rtk_api_ret_t retVal;
    ...
    if ((retVal = rtl8367c_setRealKeepRemarkEn(phydev, (rtk_uint32)enabled)) != RT_ERR_OK)
        return retVal;
 ...
 *
 */
rtk_api_ret_t rtk_vlan_realKeepRemarkEnable_get(struct phy_device *phydev, rtk_enable_t *pEnabled)
{
    rtk_api_ret_t retVal;
    ...
    if ((retVal = rtl8367c_getRealKeepRemarkEn(phydev, (rtk_uint32 *)pEnabled)) != RT_ERR_OK)
        return retVal;
 ...
 *
 */
rtk_api_ret_t rtk_vlan_reset(struct phy_device *phydev)
{
    rtk_api_ret_t retVal;
    ...
    if ((retVal = rtl8367c_resetVlan(phydev)) != RT_ERR_OK)
        return retVal;
