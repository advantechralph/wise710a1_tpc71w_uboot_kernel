...
#if 1
#define MDC_MDIO_WRITE(__phydev__, preamableLength, phyID, regID, data)	__phydev__->bus->write(__phydev__->bus, phyID, regID, data)
#define MDC_MDIO_READ(__phydev__, preamableLength, phyID, regID, pData) *pData = __phydev__->bus->read(__phydev__->bus, phyID, regID)
#else
void MDC_MDIO_READ(struct phy_device *phydev, rtk_uint32 preamableLength, rtk_uint32 phyID, rtk_uint32 regID, rtk_uint32 *data)
{
	struct mii_dev	*bus;

	bus = miiphy_get_dev_by_name("davinci_mdio");

	*data = bus->read(bus, phyID, 0, regID);
}

void MDC_MDIO_WRITE(struct phy_device *phydev, rtk_uint32 preamableLength, rtk_uint32 phyID, rtk_uint32 regID, rtk_uint32 data)
{
	struct mii_dev	*bus;

	bus = miiphy_get_dev_by_name("davinci_mdio");

	bus->write(bus, phyID, 0, regID, data);
}
#endif
...
rtk_int32 smi_read(struct phy_device *phydev, rtk_uint32 mAddrs, rtk_uint32 *rData)
{
#if (!defined(MDC_MDIO_OPERATION) && !defined(SPI_OPERATION))
    ...
    MDC_MDIO_WRITE(phydev, MDC_MDIO_PREAMBLE_LEN, phydev->addr, MDC_MDIO_CTRL0_REG, MDC_MDIO_ADDR_OP);

    /* Write address to register 23 */
    MDC_MDIO_WRITE(phydev, MDC_MDIO_PREAMBLE_LEN, phydev->addr, MDC_MDIO_ADDRESS_REG, mAddrs);

    /* Write read control code to register 21 */
    MDC_MDIO_WRITE(phydev, MDC_MDIO_PREAMBLE_LEN, phydev->addr, MDC_MDIO_CTRL1_REG, MDC_MDIO_READ_OP);

    /* Read data from register 25 */
    MDC_MDIO_READ(phydev, MDC_MDIO_PREAMBLE_LEN, phydev->addr, MDC_MDIO_DATA_READ_REG, rData);
...
rtk_int32 smi_write(struct phy_device *phydev, rtk_uint32 mAddrs, rtk_uint32 rData)
{
#if (!defined(MDC_MDIO_OPERATION) && !defined(SPI_OPERATION))
    rtk_int8 con;
    rtk_uint32 ACK;
    ...
    MDC_MDIO_WRITE(phydev, MDC_MDIO_PREAMBLE_LEN, phydev->addr, MDC_MDIO_DATA_WRITE_REG, rData);

    /* Write data control code to register 21 */
    MDC_MDIO_WRITE(phydev, MDC_MDIO_PREAMBLE_LEN, phydev->addr, MDC_MDIO_CTRL1_REG, MDC_MDIO_WRITE_OP);
