 ...
 */
ret_t rtl8367c_setAsicFieldSelector(struct phy_device *phydev, rtk_uint32 index, rtk_uint32 format, rtk_uint32 offset)
{
    rtk_uint32 regData;
    ...
    return rtl8367c_setAsicReg(phydev, RTL8367C_FIELD_SELECTOR_REG(index), regData);
}
/* Function Name:
 ...
 *      None
 */
ret_t rtl8367c_getAsicFieldSelector(struct phy_device *phydev, rtk_uint32 index, rtk_uint32* pFormat, rtk_uint32* pOffset)
{
    ret_t retVal;
    rtk_uint32 regData;

    retVal = rtl8367c_getAsicReg(phydev, RTL8367C_FIELD_SELECTOR_REG(index), &regData);
    if(retVal != RT_ERR_OK)
        return retVal;
