 ...
 */
rtk_api_ret_t rtk_trap_unknownUnicastPktAction_set(struct phy_device *phydev, rtk_port_t port, rtk_trap_ucast_action_t ucast_action)
{
    ...
    if ((retVal = rtl8367c_setAsicPortUnknownDaBehavior(phydev, rtk_switch_port_L2P_get(phydev, port), ucast_action)) != RT_ERR_OK)
        return retVal;
 ...
 *          - UCAST_ACTION_FLOODING
 */
rtk_api_ret_t rtk_trap_unknownUnicastPktAction_get(struct phy_device *phydev, rtk_port_t port, rtk_trap_ucast_action_t *pUcast_action)
{
    rtk_api_ret_t retVal;
    ...
    if ((retVal = rtl8367c_getAsicPortUnknownDaBehavior(phydev, rtk_switch_port_L2P_get(phydev, port), pUcast_action)) != RT_ERR_OK)
        return retVal;
 ...
 *          - UCAST_ACTION_TRAP2CPU
 */
rtk_api_ret_t rtk_trap_unknownMacPktAction_set(struct phy_device *phydev, rtk_trap_ucast_action_t ucast_action)
{
    rtk_api_ret_t retVal;
    ...
    if ((retVal = rtl8367c_setAsicPortUnknownSaBehavior(phydev, ucast_action)) != RT_ERR_OK)
        return retVal;
 ...
 *
 */
rtk_api_ret_t rtk_trap_unknownMacPktAction_get(struct phy_device *phydev, rtk_trap_ucast_action_t *pUcast_action)
{
    rtk_api_ret_t retVal;
    ...
    if ((retVal = rtl8367c_getAsicPortUnknownSaBehavior(phydev, pUcast_action)) != RT_ERR_OK)
        return retVal;
 ...
 *          - UCAST_ACTION_TRAP2CPU
 */
rtk_api_ret_t rtk_trap_unmatchMacPktAction_set(struct phy_device *phydev, rtk_trap_ucast_action_t ucast_action)
{
    rtk_api_ret_t retVal;
    ...
    if ((retVal = rtl8367c_setAsicPortUnmatchedSaBehavior(phydev, ucast_action)) != RT_ERR_OK)
        return retVal;
 ...
 *          - UCAST_ACTION_TRAP2CPU
 */
rtk_api_ret_t rtk_trap_unmatchMacPktAction_get(struct phy_device *phydev, rtk_trap_ucast_action_t *pUcast_action)
{
    rtk_api_ret_t retVal;
    ...
    if ((retVal = rtl8367c_getAsicPortUnmatchedSaBehavior(phydev, pUcast_action)) != RT_ERR_OK)
        return retVal;
 ...
 * Note:
 */
rtk_api_ret_t rtk_trap_unmatchMacMoving_set(struct phy_device *phydev, rtk_port_t port, rtk_enable_t enable)
{
    rtk_api_ret_t retVal;
    ...
    if ((retVal = rtl8367c_setAsicPortUnmatchedSaMoving(phydev, rtk_switch_port_L2P_get(phydev, port), enable)) != RT_ERR_OK)
        return retVal;
 ...
 * Note:
 */
rtk_api_ret_t rtk_trap_unmatchMacMoving_get(struct phy_device *phydev, rtk_port_t port, rtk_enable_t *pEnable)
{
    rtk_api_ret_t retVal;
    ...
    if ((retVal = rtl8367c_getAsicPortUnmatchedSaMoving(phydev, rtk_switch_port_L2P_get(phydev, port), pEnable)) != RT_ERR_OK)
        return retVal;
 ...
 */
rtk_api_ret_t rtk_trap_unknownMcastPktAction_set(struct phy_device *phydev, rtk_port_t port, rtk_mcast_type_t type, rtk_trap_mcast_action_t mcast_action)
{
    rtk_api_ret_t retVal;
    ...
    switch (type)
    {
        case MCAST_L2:
            ...
            if ((retVal = rtl8367c_setAsicUnknownL2MulticastBehavior(phydev, rtk_switch_port_L2P_get(phydev, port), rawAction)) != RT_ERR_OK)
                return retVal;
        ...
        case MCAST_IPV4:
            ...
            if ((retVal = rtl8367c_setAsicUnknownIPv4MulticastBehavior(phydev, rtk_switch_port_L2P_get(phydev, port), rawAction)) != RT_ERR_OK)
                return retVal;
        ...
        case MCAST_IPV6:
            ...
            if ((retVal = rtl8367c_setAsicUnknownIPv6MulticastBehavior(phydev, rtk_switch_port_L2P_get(phydev, port), rawAction)) != RT_ERR_OK)
                return retVal;
 ...
 *          - MCAST_ACTION_TRAP2CPU
 */
rtk_api_ret_t rtk_trap_unknownMcastPktAction_get(struct phy_device *phydev, rtk_port_t port, rtk_mcast_type_t type, rtk_trap_mcast_action_t *pMcast_action)
{
    rtk_api_ret_t retVal;
    rtk_uint32 rawAction;
    ...
    {
        case MCAST_L2:
            if ((retVal = rtl8367c_getAsicUnknownL2MulticastBehavior(phydev, rtk_switch_port_L2P_get(phydev, port), &rawAction)) != RT_ERR_OK)
                return retVal;
            ...
            break;
        case MCAST_IPV4:
            if ((retVal = rtl8367c_getAsicUnknownIPv4MulticastBehavior(phydev, rtk_switch_port_L2P_get(phydev, port), &rawAction)) != RT_ERR_OK)
                return retVal;

            *pMcast_action = (rtk_trap_mcast_action_t)rawAction;
            break;
        case MCAST_IPV6:
            if ((retVal = rtl8367c_getAsicUnknownIPv6MulticastBehavior(phydev, rtk_switch_port_L2P_get(phydev, port), &rawAction)) != RT_ERR_OK)
                return retVal;
 ...
 */
rtk_api_ret_t rtk_trap_lldpEnable_set(struct phy_device *phydev, rtk_enable_t enabled)
{
    rtk_api_ret_t retVal;
    ...
    if ((retVal = rtl8367c_setAsicRmaLldp(phydev, enabled, &rmacfg)) != RT_ERR_OK)
        return retVal;
 ...
 *      - 01:80:c2:00:00:00 ethertype = 0x88CC
 */
rtk_api_ret_t rtk_trap_lldpEnable_get(struct phy_device *phydev, rtk_enable_t *pEnabled)
{
    rtk_api_ret_t retVal;
    rtl8367c_rma_t rmacfg;
    ...
    if ((retVal = rtl8367c_getAsicRmaLldp(phydev, pEnabled, &rmacfg)) != RT_ERR_OK)
        return retVal;
 ...
 */
rtk_api_ret_t rtk_trap_reasonTrapToCpuPriority_set(struct phy_device *phydev, rtk_trap_reason_type_t type, rtk_pri_t priority)
{
    rtk_api_ret_t retVal;
    rtl8367c_rma_t rmacfg;
    ...
    switch (type)
    {
        case TRAP_REASON_RMA:
            if ((retVal = rtl8367c_getAsicRma(phydev, 0, &rmacfg)) != RT_ERR_OK)
                return retVal;
            rmacfg.trap_priority= priority;
            if ((retVal = rtl8367c_setAsicRma(phydev, 0, &rmacfg)) != RT_ERR_OK)
                return retVal;

            break;
        case TRAP_REASON_OAM:
            if ((retVal = rtl8367c_setAsicOamCpuPri(phydev, priority)) != RT_ERR_OK)
                return retVal;

            break;
        case TRAP_REASON_1XUNAUTH:
            if ((retVal = rtl8367c_setAsic1xTrapPriority(phydev, priority)) != RT_ERR_OK)
                return retVal;

            break;
        case TRAP_REASON_VLANSTACK:
            if ((retVal = rtl8367c_setAsicSvlanTrapPriority(phydev, priority)) != RT_ERR_OK)
                return retVal;

            break;
        case TRAP_REASON_UNKNOWNMC:
            if ((retVal = rtl8367c_setAsicUnknownMulticastTrapPriority(phydev, priority)) != RT_ERR_OK)
                return retVal;
 ...
 *      - TRAP_REASON_UNKNOWNMC
 */
rtk_api_ret_t rtk_trap_reasonTrapToCpuPriority_get(struct phy_device *phydev, rtk_trap_reason_type_t type, rtk_pri_t *pPriority)
{
    rtk_api_ret_t retVal;
    rtl8367c_rma_t rmacfg;
    ...
    switch (type)
    {
        case TRAP_REASON_RMA:
            ...
            *pPriority = rmacfg.trap_priority;
            ...
            break;
        case TRAP_REASON_OAM:
            if ((retVal = rtl8367c_getAsicOamCpuPri(phydev, pPriority)) != RT_ERR_OK)
                return retVal;

            break;
        case TRAP_REASON_1XUNAUTH:
            if ((retVal = rtl8367c_getAsic1xTrapPriority(phydev, pPriority)) != RT_ERR_OK)
                return retVal;

            break;
        case TRAP_REASON_VLANSTACK:
            if ((retVal = rtl8367c_getAsicSvlanTrapPriority(phydev, pPriority)) != RT_ERR_OK)
                return retVal;

            break;
        case TRAP_REASON_UNKNOWNMC:
            if ((retVal = rtl8367c_getAsicUnknownMulticastTrapPriority(phydev, pPriority)) != RT_ERR_OK)
                return retVal;
 ...
 */
rtk_api_ret_t rtk_trap_rmaAction_set(struct phy_device *phydev, rtk_trap_type_t type, rtk_trap_rma_action_t rma_action)
{
    rtk_api_ret_t retVal;
    rtl8367c_rma_t rmacfg;
    rtk_uint32 tmp;
 ...
 *      - RMA_ACTION_FORWARD_EXCLUDE_CPU
 */
rtk_api_ret_t rtk_trap_rmaAction_get(struct phy_device *phydev, rtk_trap_type_t type, rtk_trap_rma_action_t *pRma_action)
{
    rtk_api_ret_t retVal;
    rtl8367c_rma_t rmacfg;
    rtk_uint32 tmp;
 ...
 *      - TRAP_LLDP.
 */
rtk_api_ret_t rtk_trap_rmaKeepFormat_set(struct phy_device *phydev, rtk_trap_type_t type, rtk_enable_t enable)
{
    rtk_api_ret_t retVal;
    rtl8367c_rma_t rmacfg;
    rtk_uint32 tmp;
 ...
 *      - TRAP_LLDP.
 */
rtk_api_ret_t rtk_trap_rmaKeepFormat_get(struct phy_device *phydev, rtk_trap_type_t type, rtk_enable_t *pEnable)
{
    rtk_api_ret_t retVal;
    rtl8367c_rma_t rmacfg;
    rtk_uint32 tmp;
