 ...
 */
rtk_api_ret_t rtk_trunk_port_set(struct phy_device *phydev, rtk_trunk_group_t trk_gid, rtk_portmask_t *pTrunk_member_portmask)
{
    ...
    RTK_CHK_TRUNK_GROUP_VALID(phydev, trk_gid);

    if(NULL == pTrunk_member_portmask)
        return RT_ERR_NULL_POINTER;

    RTK_CHK_PORTMASK_VALID(phydev, pTrunk_member_portmask);

    if((retVal = rtk_switch_portmask_L2P_get(phydev, pTrunk_member_portmask, &pmsk)) != RT_ERR_OK)
        return retVal;
    ...
    if ((retVal = rtl8367c_setAsicTrunkingGroup(phydev, trk_gid, pmsk)) != RT_ERR_OK)
        return retVal;
 ...
 */
rtk_api_ret_t rtk_trunk_port_get(struct phy_device *phydev, rtk_trunk_group_t trk_gid, rtk_portmask_t *pTrunk_member_portmask)
{
    rtk_api_ret_t retVal;
    rtk_uint32 pmsk;
    ...
    if ((retVal = rtl8367c_getAsicTrunkingGroup(phydev, trk_gid, &pmsk)) != RT_ERR_OK)
        return retVal;

    pmsk = pmsk << RTL8367C_PORT_TRUNK_GROUP_MASK_OFFSET(trk_gid);

    if((retVal = rtk_switch_portmask_P2L_get(phydev, pmsk, pTrunk_member_portmask)) != RT_ERR_OK)
        return retVal;
 ...
 */
rtk_api_ret_t rtk_trunk_distributionAlgorithm_set(struct phy_device *phydev, rtk_trunk_group_t trk_gid, rtk_uint32 algo_bitmask)
{
    rtk_api_ret_t retVal;
    ...
    if ((retVal = rtl8367c_setAsicTrunkingHashSelect(phydev, algo_bitmask)) != RT_ERR_OK)
        return retVal;
 ...
 */
rtk_api_ret_t rtk_trunk_distributionAlgorithm_get(struct phy_device *phydev, rtk_trunk_group_t trk_gid, rtk_uint32 *pAlgo_bitmask)
{
    rtk_api_ret_t retVal;
    ...
    if ((retVal = rtl8367c_getAsicTrunkingHashSelect(phydev, (rtk_uint32 *)pAlgo_bitmask)) != RT_ERR_OK)
        return retVal;
 ...
 */
rtk_api_ret_t rtk_trunk_trafficSeparate_set(struct phy_device *phydev, rtk_trunk_group_t trk_gid, rtk_trunk_separateType_t separateType)
{
    rtk_api_ret_t retVal;
    rtk_uint32 enabled;
    ...
    if ((retVal = rtl8367c_setAsicTrunkingFlood(phydev, enabled)) != RT_ERR_OK)
        return retVal;
 ...
 *      SEPARATE_FLOOD: trunk MSB link up port is dedicated to TX flooding (L2 lookup miss) traffic
 */
rtk_api_ret_t rtk_trunk_trafficSeparate_get(struct phy_device *phydev, rtk_trunk_group_t trk_gid, rtk_trunk_separateType_t *pSeparateType)
{
    rtk_api_ret_t retVal;
    rtk_uint32 enabled;
    ...
    if ((retVal = rtl8367c_getAsicTrunkingFlood(phydev, &enabled)) != RT_ERR_OK)
        return retVal;
 ...
 *      - TRUNK_MODE_DUMB
 */
rtk_api_ret_t rtk_trunk_mode_set(struct phy_device *phydev, rtk_trunk_mode_t mode)
{
    rtk_api_ret_t retVal;
    ...
    if ((retVal = rtl8367c_setAsicTrunkingMode(phydev, (rtk_uint32)mode)) != RT_ERR_OK)
        return retVal;
 ...
 *      - TRUNK_MODE_DUMB
 */
rtk_api_ret_t rtk_trunk_mode_get(struct phy_device *phydev, rtk_trunk_mode_t *pMode)
{
    rtk_api_ret_t retVal;
    ...
    if ((retVal = rtl8367c_getAsicTrunkingMode(phydev, (rtk_uint32 *)pMode)) != RT_ERR_OK)
        return retVal;
 ...
 *      None.
 */
rtk_api_ret_t rtk_trunk_trafficPause_set(struct phy_device *phydev, rtk_trunk_group_t trk_gid, rtk_enable_t enable)
{
    rtk_api_ret_t retVal;
    ...
    if ((retVal = rtl8367c_setAsicTrunkingFc(phydev, (rtk_uint32)trk_gid, (rtk_uint32)enable)) != RT_ERR_OK)
        return retVal;
 ...
 *      None.
 */
rtk_api_ret_t rtk_trunk_trafficPause_get(struct phy_device *phydev, rtk_trunk_group_t trk_gid, rtk_enable_t *pEnable)
{
    rtk_api_ret_t retVal;
    ...
    if ((retVal = rtl8367c_getAsicTrunkingFc(phydev, (rtk_uint32)trk_gid, (rtk_uint32 *)pEnable)) != RT_ERR_OK)
        return retVal;
 ...
 */
rtk_api_ret_t rtk_trunk_hashMappingTable_set(struct phy_device *phydev, rtk_trunk_group_t trk_gid, rtk_trunk_hashVal2Port_t *pHash2Port_array)
{
    rtk_api_ret_t retVal;
    ...
    if(trk_gid <= TRUNK_GROUP1)
    {
        ...
        {
            if ((retVal = rtl8367c_setAsicTrunkingHashTable(phydev, hashValue, pHash2Port_array->value[hashValue])) != RT_ERR_OK)
                return retVal;
        }
    }
    else
    {
        for(hashValue = 0; hashValue < RTK_MAX_NUM_OF_TRUNK_HASH_VAL; hashValue++)
        {
            if ((retVal = rtl8367c_setAsicTrunkingHashTable1(phydev, hashValue, pHash2Port_array->value[hashValue])) != RT_ERR_OK)
                return retVal;
        }
    }
 ...
 *      Trunk group 2 uses a independent table.
 */
rtk_api_ret_t rtk_trunk_hashMappingTable_get(struct phy_device *phydev, rtk_trunk_group_t trk_gid, rtk_trunk_hashVal2Port_t *pHash2Port_array)
{
    rtk_api_ret_t retVal;
    rtk_uint32 hashValue;
    rtk_uint32 hashPort;
    ...
    if(trk_gid <= TRUNK_GROUP1)
    {
        for(hashValue = 0; hashValue < RTK_MAX_NUM_OF_TRUNK_HASH_VAL; hashValue++)
        {
            if ((retVal = rtl8367c_getAsicTrunkingHashTable(phydev, hashValue, &hashPort)) != RT_ERR_OK)
                return retVal;
    ...
    else
    {
        for(hashValue = 0; hashValue < RTK_MAX_NUM_OF_TRUNK_HASH_VAL; hashValue++)
        {
            if ((retVal = rtl8367c_getAsicTrunkingHashTable1(phydev, hashValue, &hashPort)) != RT_ERR_OK)
                return retVal;
 ...
 *      None.
 */
rtk_api_ret_t rtk_trunk_portQueueEmpty_get(struct phy_device *phydev, rtk_portmask_t *pEmpty_portmask)
{
    rtk_api_ret_t retVal;
    rtk_uint32 pmask;
    ...
    if ((retVal = rtl8367c_getAsicQeueuEmptyStatus(phydev, &pmask)) != RT_ERR_OK)
        return retVal;

    if ((retVal = rtk_switch_portmask_P2L_get(phydev, pmask, pEmpty_portmask)) != RT_ERR_OK)
        return retVal;
