...
static rtk_api_ret_t _rtk_filter_igrAcl_writeDataField(struct phy_device *phydev, rtl8367c_aclrule *aclRule, rtk_filter_field_t *fieldPtr);
 ...
 */
rtk_api_ret_t rtk_filter_igrAcl_init(struct phy_device *phydev)
{
    ...
    if ((ret = rtk_filter_igrAcl_cfg_delAll(phydev)) != RT_ERR_OK)
        return ret;
    ...
    for(i = 0; i < RTL8367C_ACLTEMPLATENO; i++)
    {
        ...
        if ((ret = rtl8367c_setAsicAclTemplate(phydev, i, &aclTemp)) != RT_ERR_OK)
            return ret;
    }

    for(i = 0; i < RTL8367C_FIELDSEL_FORMAT_NUMBER; i++)
    {
        if ((ret = rtl8367c_setAsicFieldSelector(phydev, i, field_selector[i][0], field_selector[i][1])) != RT_ERR_OK)
            return ret;
    }

    RTK_SCAN_ALL_PHY_PORTMASK(phydev, i)
    {
        if ((ret = rtl8367c_setAsicAcl(phydev, i, TRUE)) != RT_ERR_OK)
            return ret;

        if ((ret = rtl8367c_setAsicAclUnmatchedPermit(phydev, i, TRUE)) != RT_ERR_OK)
            return ret;
    }
 ...
 */
rtk_api_ret_t rtk_filter_igrAcl_field_add(struct phy_device *phydev, rtk_filter_cfg_t* pFilter_cfg, rtk_filter_field_t* pFilter_field)
{
    rtk_uint32 i;
 ...
 */
rtk_api_ret_t rtk_filter_igrAcl_cfg_add(struct phy_device *phydev, rtk_filter_id_t filter_id, rtk_filter_cfg_t* pFilter_cfg, rtk_filter_action_t* pFilter_action, rtk_filter_number_t *ruleNum)
{
    ...
    while(NULL != fieldPtr)
    {
        _rtk_filter_igrAcl_writeDataField(phydev, aclRule, fieldPtr);
    ...
    *ruleNum = noRulesAdd;
    ...
    RTK_CHK_PORTMASK_VALID(phydev, &pFilter_cfg->activeport.value);
    RTK_CHK_PORTMASK_VALID(phydev, &pFilter_cfg->activeport.mask);

    for(i = 0; i < RTL8367C_ACLTEMPLATENO; i++)
    {
        if(TRUE == aclRule[i].valid)
        {
            if(rtk_switch_portmask_L2P_get(phydev, &pFilter_cfg->activeport.value, &portmask) != RT_ERR_OK)
                return RT_ERR_PORT_MASK;

            aclRule[i].data_bits.active_portmsk = portmask;

            if(rtk_switch_portmask_L2P_get(phydev, &pFilter_cfg->activeport.mask, &portmask) != RT_ERR_OK)
                return RT_ERR_PORT_MASK;
    ...
    for(actType = 0; actType < FILTER_ENACT_END; actType ++)
    {
        if(pFilter_action->actEnable[actType])
        {
            ...
            case FILTER_ENACT_CVLAN_INGRESS:
                ...
                if((retVal = rtk_vlan_checkAndCreateMbr(phydev, pFilter_action->filterCvlanVid, &vidx)) != RT_ERR_OK)
                {
                    return retVal;
                }
            ...
            case FILTER_ENACT_CVLAN_EGRESS:
             ...
             case FILTER_ENACT_POLICING_1:
                if(pFilter_action->filterPolicingIdx[1] >= (RTK_METER_NUM(phydev) + RTL8367C_MAX_LOG_CNT_NUM))
                    return RT_ERR_INPUT;
            ...
            case FILTER_ENACT_SVLAN_EGRESS:
                ...
                if((retVal = rtk_svlan_checkAndCreateMbr(phydev, pFilter_action->filterSvlanVid, &svidx)) != RT_ERR_OK)
                    return retVal;
            ...
            case FILTER_ENACT_POLICING_2:
                if(pFilter_action->filterPolicingIdx[2] >= (RTK_METER_NUM(phydev) + RTL8367C_MAX_LOG_CNT_NUM))
                    return RT_ERR_INPUT;
            ...
            case FILTER_ENACT_POLICING_0:
                if(pFilter_action->filterPolicingIdx[0] >= (RTK_METER_NUM(phydev) + RTL8367C_MAX_LOG_CNT_NUM))
                    return RT_ERR_INPUT;
            ...
            case FILTER_ENACT_POLICING_3:
                if(pFilter_action->filterPriority >= (RTK_METER_NUM(phydev) + RTL8367C_MAX_LOG_CNT_NUM))
                    return RT_ERR_INPUT;
            ...
            case FILTER_ENACT_REDIRECT:
                RTK_CHK_PORTMASK_VALID(phydev, &pFilter_action->filterPortmask);

                aclAct.fwdact = FILTER_ENACT_FWD_TYPE(actType);
                aclAct.fwdact_ext = FALSE;

                if(rtk_switch_portmask_L2P_get(phydev, &pFilter_action->filterPortmask, &portmask) != RT_ERR_OK)
                    return RT_ERR_PORT_MASK;
            ...
            case FILTER_ENACT_COPY_CPU:
                if((retVal = rtl8367c_getAsicCputagTrapPort(phydev, &cpuPort)) != RT_ERR_OK)
                    return retVal;
    ...
    {
        if((retVal = rtl8367c_getAsicAclRule(phydev, i, &tempRule)) != RT_ERR_OK )
            return retVal;
    ...
    for(i = 0; i < RTL8367C_ACLTEMPLATENO; i++)
    {
        if(aclRule[i].valid == TRUE)
        {
            ...
            if((retVal = rtl8367c_setAsicAclActCtrl(phydev, filter_id + ruleId, aclActCtrl)) != RT_ERR_OK )
                return retVal;
            /* write ACL action */
            if((retVal = rtl8367c_setAsicAclAct(phydev, filter_id + ruleId, &aclAct)) != RT_ERR_OK )
                return retVal;

            /* write ACL not */
            if((retVal = rtl8367c_setAsicAclNot(phydev, filter_id + ruleId, pFilter_cfg->invert)) != RT_ERR_OK )
                return retVal;
            /* write ACL rule */
            if((retVal = rtl8367c_setAsicAclRule(phydev, filter_id + ruleId, &aclRule[i])) != RT_ERR_OK )
                return retVal;
 ...
 */
rtk_api_ret_t rtk_filter_igrAcl_cfg_del(struct phy_device *phydev, rtk_filter_id_t filter_id)
{
    ...
    if((ret = rtl8367c_setAsicAclRule(phydev, filter_id, &initRule)) != RT_ERR_OK)
        return ret;
    if((ret = rtl8367c_setAsicAclActCtrl(phydev, filter_id, FILTER_ENACT_INIT_MASK))!= RT_ERR_OK)
        return ret;
    if((ret = rtl8367c_setAsicAclAct(phydev, filter_id, &initAct)) != RT_ERR_OK)
        return ret;
    if((ret = rtl8367c_setAsicAclNot(phydev, filter_id, DISABLED)) != RT_ERR_OK )
        return ret;
 ...
 */
rtk_api_ret_t rtk_filter_igrAcl_cfg_delAll(struct phy_device *phydev)
{
    rtk_uint32            i;
    rtk_api_ret_t     ret;
    ...
    {
        if((ret = rtl8367c_setAsicAclActCtrl(phydev, i, FILTER_ENACT_INIT_MASK))!= RT_ERR_OK)
            return ret;
        if((ret = rtl8367c_setAsicAclNot(phydev, i, DISABLED)) != RT_ERR_OK )
            return ret;
    }

    return rtl8367c_setAsicRegBit(phydev, RTL8367C_REG_ACL_RESET_CFG, RTL8367C_ACL_RESET_CFG_OFFSET, TRUE);;
}
 ...
 */
rtk_api_ret_t rtk_filter_igrAcl_cfg_get(struct phy_device *phydev, rtk_filter_id_t filter_id, rtk_filter_cfg_raw_t *pFilter_cfg, rtk_filter_action_t *pAction)
{
    rtk_api_ret_t               retVal;
    rtk_uint32                  i, tmp;
    ...
    if ((retVal = rtl8367c_getAsicAclRule(phydev, filter_id, &aclRule)) != RT_ERR_OK)
        return retVal;
    ...
    if(rtk_switch_portmask_P2L_get(phydev, phyPmask,&(pFilter_cfg->activeport.value)) != RT_ERR_OK)
        return RT_ERR_FAILED;

    phyPmask = aclRule.care_bits.active_portmsk;
    if(rtk_switch_portmask_P2L_get(phydev, phyPmask,&(pFilter_cfg->activeport.mask)) != RT_ERR_OK)
        return RT_ERR_FAILED;
    ...
    if ((retVal = rtl8367c_getAsicAclNot(phydev, filter_id, &tmp))!= RT_ERR_OK)
        return retVal;
    ...
    if ((retVal = rtl8367c_getAsicAclActCtrl(phydev, filter_id, &tmp))!= RT_ERR_OK)
        return retVal;

    if ((retVal = rtl8367c_getAsicAclAct(phydev, filter_id, &aclAct)) != RT_ERR_OK)
        return retVal;
    ...
    if(tmp & FILTER_ENACT_FWD_MASK)
    {
        if(TRUE == aclAct.fwdact_ext)
        {
            ...
            if(rtk_switch_portmask_P2L_get(phydev, phyPmask,&(pAction->filterPortmask)) != RT_ERR_OK)
                return RT_ERR_FAILED;
        }
    ...
    if(tmp & FILTER_ENACT_SVLAN_MASK)
    {
        ...
        {
            if((retVal = rtl8367c_getAsicSvlanMemberConfiguration(phydev, aclAct.svidx_sact, &svlan_cfg)) != RT_ERR_OK)
                return retVal;
    ...
    if(tmp & FILTER_ENACT_CVLAN_MASK)
    {
            ...
            FILTER_ENACT_CACTEXT_BOTHVLANTAG == aclAct.cact_ext )
        {
            ...
            {
                if((retVal = rtl8367c_getAsicVlanMemberConfig(phydev, aclAct.cvidx_cact, &vlanMC)) != RT_ERR_OK)
                    return retVal;
    ...
    if ((retVal = rtl8367c_getAsicAclTemplate(phydev, aclRule.data_bits.type, &type))!= RT_ERR_OK)
        return retVal;
 ...
 */
rtk_api_ret_t rtk_filter_igrAcl_unmatchAction_set(struct phy_device *phydev, rtk_port_t port, rtk_filter_unmatch_action_t action)
{
    rtk_api_ret_t ret;
    ...
    if((ret = rtl8367c_setAsicAclUnmatchedPermit(phydev, rtk_switch_port_L2P_get(phydev, port), action)) != RT_ERR_OK)
       return ret;
 ...
 */
rtk_api_ret_t rtk_filter_igrAcl_unmatchAction_get(struct phy_device *phydev, rtk_port_t port, rtk_filter_unmatch_action_t* pAction)
{
    rtk_api_ret_t ret;
    ...
    if((ret = rtl8367c_getAsicAclUnmatchedPermit(phydev, rtk_switch_port_L2P_get(phydev, port), pAction)) != RT_ERR_OK)
       return ret;
 ...
 *      This function gets action of packets when no ACL configruation matches.
 */
rtk_api_ret_t rtk_filter_igrAcl_state_set(struct phy_device *phydev, rtk_port_t port, rtk_filter_state_t state)
{
    rtk_api_ret_t ret;
    ...
    if((ret = rtl8367c_setAsicAcl(phydev, rtk_switch_port_L2P_get(phydev, port), state)) != RT_ERR_OK)
       return ret;
 ...
 *      This function gets action of packets when no ACL configruation matches.
 */
rtk_api_ret_t rtk_filter_igrAcl_state_get(struct phy_device *phydev, rtk_port_t port, rtk_filter_state_t* pState)
{
    rtk_api_ret_t ret;
    ...
    if((ret = rtl8367c_getAsicAcl(phydev, rtk_switch_port_L2P_get(phydev, port), pState)) != RT_ERR_OK)
       return ret;
 ...
 */
rtk_api_ret_t rtk_filter_igrAcl_template_set(struct phy_device *phydev, rtk_filter_template_t *aclTemplate)
{
    rtk_api_ret_t retVal;
    rtk_uint32 idxField;
    ...
    if((retVal = rtl8367c_setAsicAclTemplate(phydev, aclTemplate->index, &aclType)) != RT_ERR_OK)
        return retVal;
 ...
 */
rtk_api_ret_t rtk_filter_igrAcl_template_get(struct phy_device *phydev, rtk_filter_template_t *aclTemplate)
{
    rtk_api_ret_t ret;
    rtk_uint32 idxField;
    rtl8367c_acltemplate_t aclType;
   ...
   if((ret = rtl8367c_getAsicAclTemplate(phydev, aclTemplate->index, &aclType)) != RT_ERR_OK)
       return ret;
 ...
 */
rtk_api_ret_t rtk_filter_igrAcl_field_sel_set(struct phy_device *phydev, rtk_uint32 index, rtk_field_sel_t format, rtk_uint32 offset)
{
    rtk_api_ret_t ret;
    ...
    if((ret = rtl8367c_setAsicFieldSelector(phydev, index, (rtk_uint32)format, offset)) != RT_ERR_OK)
       return ret;
 ...
 *      None.
 */
rtk_api_ret_t rtk_filter_igrAcl_field_sel_get(struct phy_device *phydev, rtk_uint32 index, rtk_field_sel_t *pFormat, rtk_uint32 *pOffset)
{
    rtk_api_ret_t ret;
    ...
    if((ret = rtl8367c_getAsicFieldSelector(phydev, index, pFormat, pOffset)) != RT_ERR_OK)
       return ret;
 ...
 */
rtk_api_ret_t rtk_filter_iprange_set(struct phy_device *phydev, rtk_uint32 index, rtk_filter_iprange_t type, ipaddr_t upperIp, ipaddr_t lowerIp)
{
    rtk_api_ret_t ret;
    ...
    if((ret = rtl8367c_setAsicAclIpRange(phydev, index, type, upperIp, lowerIp)) != RT_ERR_OK)
       return ret;
 ...
 *      None.
 */
rtk_api_ret_t rtk_filter_iprange_get(struct phy_device *phydev, rtk_uint32 index, rtk_filter_iprange_t *pType, ipaddr_t *pUpperIp, ipaddr_t *pLowerIp)
{
    rtk_api_ret_t ret;
    ...
    if((ret = rtl8367c_getAsicAclIpRange(phydev, index, pType, pUpperIp, pLowerIp)) != RT_ERR_OK)
       return ret;
 ...
 */
rtk_api_ret_t rtk_filter_vidrange_set(struct phy_device *phydev, rtk_uint32 index, rtk_filter_vidrange_t type, rtk_uint32 upperVid, rtk_uint32 lowerVid)
{
    rtk_api_ret_t ret;
    ...
    if((ret = rtl8367c_setAsicAclVidRange(phydev, index, type, upperVid, lowerVid)) != RT_ERR_OK)
       return ret;
 ...
 *      None.
 */
rtk_api_ret_t rtk_filter_vidrange_get(struct phy_device *phydev, rtk_uint32 index, rtk_filter_vidrange_t *pType, rtk_uint32 *pUpperVid, rtk_uint32 *pLowerVid)
{
    rtk_api_ret_t ret;
    ...
    if((ret = rtl8367c_getAsicAclVidRange(phydev, index, pType, pUpperVid, pLowerVid)) != RT_ERR_OK)
       return ret;
 ...
 */
rtk_api_ret_t rtk_filter_portrange_set(struct phy_device *phydev, rtk_uint32 index, rtk_filter_portrange_t type, rtk_uint32 upperPort, rtk_uint32 lowerPort)
{
    rtk_api_ret_t ret;
    ...
    if((ret = rtl8367c_setAsicAclPortRange(phydev, index, type, upperPort, lowerPort)) != RT_ERR_OK)
       return ret;
 ...
 *      None.
 */
rtk_api_ret_t rtk_filter_portrange_get(struct phy_device *phydev, rtk_uint32 index, rtk_filter_portrange_t *pType, rtk_uint32 *pUpperPort, rtk_uint32 *pLowerPort)
{
    rtk_api_ret_t ret;
    ...
    if((ret = rtl8367c_getAsicAclPortRange(phydev, index, pType, pUpperPort, pLowerPort)) != RT_ERR_OK)
       return ret;
 ...
 *      none
 */
rtk_api_ret_t rtk_filter_igrAclPolarity_set(struct phy_device *phydev, rtk_uint32 polarity)
{
    /* Check initialization state */
    RTK_CHK_INIT_STATE(phydev);
    ...
    if(polarity > 1)
        return RT_ERR_OUT_OF_RANGE;
    return rtl8367c_setAsicAclGpioPolarity(phydev, polarity);
}
/* Function Name:
 ...
 *      none
 */
rtk_api_ret_t rtk_filter_igrAclPolarity_get(struct phy_device *phydev, rtk_uint32* pPolarity)
{
    /* Check initialization state */
    RTK_CHK_INIT_STATE(phydev);
    ...
    return rtl8367c_getAsicAclGpioPolarity(phydev, pPolarity);
}
