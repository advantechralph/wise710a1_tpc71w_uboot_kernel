 ...
 */
rtk_api_ret_t rtk_int_polarity_set(struct phy_device *phydev, rtk_int_polarity_t type)
{
    ...
    if ((retVal = rtl8367c_setAsicInterruptPolarity(phydev, type)) != RT_ERR_OK)
        return retVal;
 ...
 */
rtk_api_ret_t rtk_int_polarity_get(struct phy_device *phydev, rtk_int_polarity_t *pType)
{
    rtk_api_ret_t retVal;
    ...
    if ((retVal = rtl8367c_getAsicInterruptPolarity(phydev, pType)) != RT_ERR_OK)
        return retVal;
 ...
 *      - INT_TYPE_SLIENT
 */
rtk_api_ret_t rtk_int_control_set(struct phy_device *phydev, rtk_int_type_t type, rtk_enable_t enable)
{
    rtk_api_ret_t retVal;
    rtk_uint32 mask;
    ...
    if ((retVal = rtl8367c_getAsicInterruptMask(phydev, &mask)) != RT_ERR_OK)
        return retVal;
    ...
    if ((retVal = rtl8367c_setAsicInterruptMask(phydev, mask)) != RT_ERR_OK)
        return retVal;
 ...
 *      - INT_TYPE_SLIENT
 */
rtk_api_ret_t rtk_int_control_get(struct phy_device *phydev, rtk_int_type_t type, rtk_enable_t *pEnable)
{
    rtk_api_ret_t retVal;
    rtk_uint32 mask;
 ...
 */
rtk_api_ret_t rtk_int_status_set(struct phy_device *phydev, rtk_int_status_t *pStatusMask)
{
    rtk_api_ret_t retVal;
    ...
    if ((retVal = rtl8367c_setAsicInterruptStatus(phydev, (rtk_uint32)pStatusMask->value[0]))!=RT_ERR_OK)
        return retVal;
 ...
 *
 */
rtk_api_ret_t rtk_int_status_get(struct phy_device *phydev, rtk_int_status_t* pStatusMask)
{
    rtk_api_ret_t   retVal;
    rtk_uint32          ims_mask;
    ...
    if ((retVal = rtl8367c_getAsicInterruptStatus(phydev, &ims_mask)) != RT_ERR_OK)
        return retVal;
 ...
 *      The status will be cleared after execute this API.
 */
rtk_api_ret_t rtk_int_advanceInfo_get(struct phy_device *phydev, rtk_int_advType_t adv_type, rtk_int_info_t *pInfo)
{
    rtk_api_ret_t   retVal;
    rtk_uint32      data;
    ...
    switch(adv_type)
    {
        case ADV_L2_LEARN_PORT_MASK:
            ...
            if((retVal = rtl8367c_getAsicInterruptRelatedStatus(phydev, intAdvType, &data)) != RT_ERR_OK)
                return retVal;

            /* Clear Advanced Info */
            if((retVal = rtl8367c_setAsicInterruptRelatedStatus(phydev, intAdvType, 0xFFFF)) != RT_ERR_OK)
                return retVal;

            /* Translate to logical portmask */
            if((retVal = rtk_switch_portmask_P2L_get(phydev, data, &(pInfo->portMask))) != RT_ERR_OK)
                return retVal;

            /* Get system learn */
            if((retVal = rtl8367c_getAsicInterruptRelatedStatus(phydev, INTRST_SYS_LEARN, &data)) != RT_ERR_OK)
                return retVal;

            /* Clear system learn */
            if((retVal = rtl8367c_setAsicInterruptRelatedStatus(phydev, INTRST_SYS_LEARN, 0x0001)) != RT_ERR_OK)
                return retVal;
        ...
        case ADV_METER_EXCEED_MASK:
            /* Get Meter Mask */
            if((retVal = rtl8367c_getAsicInterruptRelatedStatus(phydev, INTRST_METER0_15, &data)) != RT_ERR_OK)
                return retVal;

            /* Clear Advanced Info */
            if((retVal = rtl8367c_setAsicInterruptRelatedStatus(phydev, INTRST_METER0_15, 0xFFFF)) != RT_ERR_OK)
                return retVal;

            pInfo->meterMask = data & 0xFFFF;

            /* Get Meter Mask */
            if((retVal = rtl8367c_getAsicInterruptRelatedStatus(phydev, INTRST_METER16_31, &data)) != RT_ERR_OK)
                return retVal;

            /* Clear Advanced Info */
            if((retVal = rtl8367c_setAsicInterruptRelatedStatus(phydev, INTRST_METER16_31, 0xFFFF)) != RT_ERR_OK)
                return retVal;
