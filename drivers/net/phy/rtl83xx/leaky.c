 ...
 *      - LEAKY_LLDP.
 */
rtk_api_ret_t rtk_leaky_vlan_set(struct phy_device *phydev, rtk_leaky_type_t type, rtk_enable_t enable)
{
    ...
    else if (LEAKY_IPMULTICAST == type)
    {
        ...
        {
            if ((retVal = rtl8367c_setAsicIpMulticastVlanLeaky(phydev, port,enable)) != RT_ERR_OK)
                return retVal;
        }
    }
    else if (LEAKY_IGMP == type)
    {
        if ((retVal = rtl8367c_setAsicIGMPVLANLeaky(phydev, enable)) != RT_ERR_OK)
            return retVal;
    }
 ...
 *      - LEAKY_LLDP.
 */
rtk_api_ret_t rtk_leaky_vlan_get(struct phy_device *phydev, rtk_leaky_type_t type, rtk_enable_t *pEnable)
{
    rtk_api_ret_t retVal;
    rtk_uint32 port,tmp;
    rtl8367c_rma_t rmacfg;
    ...
    else if (LEAKY_IPMULTICAST == type)
    {
        for (port = 0; port <= RTK_PORT_ID_MAX; port++)
        {
            if ((retVal = rtl8367c_getAsicIpMulticastVlanLeaky(phydev, port, &tmp)) != RT_ERR_OK)
                return retVal;
    ...
    {
        if ((retVal = rtl8367c_getAsicIGMPVLANLeaky(phydev, &tmp)) != RT_ERR_OK)
            return retVal;
 ...
 *      - LEAKY_LLDP.
 */
rtk_api_ret_t rtk_leaky_portIsolation_set(struct phy_device *phydev, rtk_leaky_type_t type, rtk_enable_t enable)
{
    rtk_api_ret_t retVal;
    rtk_uint32 port;
    rtl8367c_rma_t rmacfg;
    ...
    else if (LEAKY_IPMULTICAST == type)
    {
        ...
        {
            if ((retVal = rtl8367c_setAsicIpMulticastPortIsoLeaky(phydev, port,enable)) != RT_ERR_OK)
                return retVal;
        }
    }
    else if (LEAKY_IGMP == type)
    {
        if ((retVal = rtl8367c_setAsicIGMPIsoLeaky(phydev, enable)) != RT_ERR_OK)
            return retVal;
    }
    else if (LEAKY_CDP == type)
    {
 ...
 *      - LEAKY_LLDP.
 */
rtk_api_ret_t rtk_leaky_portIsolation_get(struct phy_device *phydev, rtk_leaky_type_t type, rtk_enable_t *pEnable)
{
    rtk_api_ret_t retVal;
    rtk_uint32 port, tmp;
    rtl8367c_rma_t rmacfg;
    ...
    else if (LEAKY_IPMULTICAST == type)
    {
        for (port = 0; port < RTK_MAX_NUM_OF_PORT; port++)
        {
            if ((retVal = rtl8367c_getAsicIpMulticastPortIsoLeaky(phydev, port, &tmp)) != RT_ERR_OK)
                return retVal;
            if (port > 0 &&(tmp != *pEnable))
                return RT_ERR_FAILED;
            *pEnable = tmp;
    ...
    {
        if ((retVal = rtl8367c_getAsicIGMPIsoLeaky(phydev, &tmp)) != RT_ERR_OK)
            return retVal;
