...
static rtk_api_ret_t _rtk_port_FiberModeAbility_set(struct phy_device *phydev, rtk_port_t port, rtk_port_phy_ability_t *pAbility)
{
    rtk_api_ret_t   retVal;
    rtk_uint32      regData;

    /* Check Combo port or not */
    RTK_CHK_PORT_IS_COMBO(phydev, port);

    /* Flow Control */
    if ((retVal = rtl8367c_getAsicReg(phydev, RTL8367C_REG_FIB0_CFG04, &regData)) != RT_ERR_OK)
        return retVal;
    ...
    if ((retVal = rtl8367c_setAsicReg(phydev, RTL8367C_REG_FIB0_CFG04, regData)) != RT_ERR_OK)
        return retVal;

    /* Speed ability */
    if( (pAbility->Full_1000 == 1) && (pAbility->Full_100 == 1) && (pAbility->AutoNegotiation == 1) )
    {
        if ((retVal = rtl8367c_setAsicRegBit(phydev, RTL8367C_REG_FIBER_CFG_1, RTL8367C_SDS_FRC_MODE_OFFSET, 0)) != RT_ERR_OK)
            return retVal;

        if ((retVal = rtl8367c_setAsicRegBits(phydev, RTL8367C_REG_FIBER_CFG_1, RTL8367C_SDS_MODE_MASK, 7)) != RT_ERR_OK)
            return retVal;

        if ((retVal = rtl8367c_setAsicReg(phydev, RTL8367C_REG_FIB0_CFG00, 0x1140)) != RT_ERR_OK)
            return retVal;
    }
    else if(pAbility->Full_1000 == 1)
    {
        if ((retVal = rtl8367c_setAsicRegBit(phydev, RTL8367C_REG_FIBER_CFG_1, RTL8367C_SDS_FRC_MODE_OFFSET, 1)) != RT_ERR_OK)
            return retVal;

        if ((retVal = rtl8367c_setAsicRegBits(phydev, RTL8367C_REG_FIBER_CFG_1, RTL8367C_SDS_MODE_MASK, 4)) != RT_ERR_OK)
            return retVal;
        ...
        {
            if ((retVal = rtl8367c_setAsicReg(phydev, RTL8367C_REG_FIB0_CFG00, 0x0140)) != RT_ERR_OK)
                return retVal;
        }
    }
    else if(pAbility->Full_100 == 1)
    {
        ...
        if ((retVal = rtl8367c_setAsicRegBits(phydev, RTL8367C_REG_FIBER_CFG_1, RTL8367C_SDS_MODE_MASK, 5)) != RT_ERR_OK)
            return retVal;

        if ((retVal = rtl8367c_setAsicReg(phydev, RTL8367C_REG_FIB0_CFG00, 0x2100)) != RT_ERR_OK)
            return retVal;
    }

    /* Digital software reset */
    if ((retVal = rtl8367c_setAsicReg(phydev, RTL8367C_REG_SDS_INDACS_ADR, 0x0003)) != RT_ERR_OK)
        return retVal;

    if ((retVal = rtl8367c_setAsicReg(phydev, RTL8367C_REG_SDS_INDACS_CMD, 0x0080)) != RT_ERR_OK)
        return retVal;

    if ((retVal = rtl8367c_getAsicReg(phydev, RTL8367C_REG_SDS_INDACS_DATA, &regData)) != RT_ERR_OK)
        return retVal;

    regData |= (0x0001 << 6);

    if ((retVal = rtl8367c_setAsicReg(phydev, RTL8367C_REG_SDS_INDACS_DATA, regData)) != RT_ERR_OK)
        return retVal;

    if ((retVal = rtl8367c_setAsicReg(phydev, RTL8367C_REG_SDS_INDACS_ADR, 0x0003)) != RT_ERR_OK)
        return retVal;

    if ((retVal = rtl8367c_setAsicReg(phydev, RTL8367C_REG_SDS_INDACS_CMD, 0x00C0)) != RT_ERR_OK)
        return retVal;
    ...
    /* CDR reset */
    if ((retVal = rtl8367c_setAsicReg(phydev, RTL8367C_REG_SDS_INDACS_DATA, 0x1401))!=RT_ERR_OK)
        return retVal;

    if ((retVal = rtl8367c_setAsicReg(phydev, RTL8367C_REG_SDS_INDACS_ADR, 0x0000))!=RT_ERR_OK)
        return retVal;

    if ((retVal = rtl8367c_setAsicReg(phydev, RTL8367C_REG_SDS_INDACS_CMD, 0x00C0))!=RT_ERR_OK)
        return retVal;

    if ((retVal = rtl8367c_setAsicReg(phydev, RTL8367C_REG_SDS_INDACS_DATA, 0x1403))!=RT_ERR_OK)
        return retVal;
...
static rtk_api_ret_t _rtk_port_FiberModeAbility_get(struct phy_device *phydev, rtk_port_t port, rtk_port_phy_ability_t *pAbility)
{
    rtk_api_ret_t   retVal;
    ...
    /* Flow Control */
    if ((retVal = rtl8367c_setAsicRegBit(phydev, RTL8367C_REG_FIBER_CFG_1, RTL8367C_SDS_FRC_REG4_OFFSET, 1)) != RT_ERR_OK)
        return retVal;

    if ((retVal = rtl8367c_setAsicRegBit(phydev, RTL8367C_REG_FIBER_CFG_1, RTL8367C_SDS_FRC_REG4_FIB100_OFFSET, 0)) != RT_ERR_OK)
        return retVal;

    if ((retVal = rtl8367c_setAsicReg(phydev, RTL8367C_REG_SDS_INDACS_ADR, 0x0044)) != RT_ERR_OK)
        return retVal;
    ...
    /* Speed ability */
    if ((retVal = rtl8367c_getAsicRegBit(phydev, RTL8367C_REG_FIBER_CFG_1, RTL8367C_SDS_FRC_MODE_OFFSET, &data)) != RT_ERR_OK)
            return retVal;
    ...
    {
        if ((retVal = rtl8367c_getAsicRegBits(phydev, RTL8367C_REG_FIBER_CFG_1, RTL8367C_SDS_MODE_MASK, &data)) != RT_ERR_OK)
            return retVal;
        ...
        if(data == 4)
        {
            ...
            if ((retVal = rtl8367c_getAsicReg(phydev, RTL8367C_REG_FIB0_CFG00, &data)) != RT_ERR_OK)
                return retVal;
 ...
 */
rtk_api_ret_t rtk_port_phyAutoNegoAbility_set(struct phy_device *phydev, rtk_port_t port, rtk_port_phy_ability_t *pAbility)
{
    rtk_api_ret_t       retVal;
    rtk_uint32          phyData;
    ...
    if (rtk_switch_isComboPort(phydev, port) == RT_ERR_OK)
    {
        if ((retVal = rtk_port_phyComboPortMedia_get(phydev, port, &media_type)) != RT_ERR_OK)
            return retVal;

        if(media_type == PORT_MEDIA_FIBER)
        {
            return _rtk_port_FiberModeAbility_set(phydev, port, pAbility);
        }
    }
    ...
    if ((retVal = rtl8367c_getAsicPHYReg(phydev, rtk_switch_port_L2P_get(phydev, port), PHY_1000_BASET_CONTROL_REG, &phyData)) != RT_ERR_OK)
        return retVal;

    phyData = (phyData & (~0x0200)) | phyEnMsk9 ;

    if ((retVal = rtl8367c_setAsicPHYReg(phydev, rtk_switch_port_L2P_get(phydev, port), PHY_1000_BASET_CONTROL_REG, phyData)) != RT_ERR_OK)
        return retVal;

    /*Auto-Negotiation control register setting*/
    if ((retVal = rtl8367c_getAsicPHYReg(phydev, rtk_switch_port_L2P_get(phydev, port), PHY_AN_ADVERTISEMENT_REG, &phyData)) != RT_ERR_OK)
        return retVal;

    phyData = (phyData & (~0x0DE0)) | phyEnMsk4;
    if ((retVal = rtl8367c_setAsicPHYReg(phydev, rtk_switch_port_L2P_get(phydev, port), PHY_AN_ADVERTISEMENT_REG, phyData)) != RT_ERR_OK)
        return retVal;

    /*Control register setting and restart auto*/
    if ((retVal = rtl8367c_getAsicPHYReg(phydev, rtk_switch_port_L2P_get(phydev, port), PHY_CONTROL_REG, &phyData)) != RT_ERR_OK)
        return retVal;
    ...
    if ((retVal = rtl8367c_setAsicPHYReg(phydev, rtk_switch_port_L2P_get(phydev, port), PHY_CONTROL_REG, phyData)) != RT_ERR_OK)
        return retVal;
 ...
 */
rtk_api_ret_t rtk_port_phyAutoNegoAbility_get(struct phy_device *phydev, rtk_port_t port, rtk_port_phy_ability_t *pAbility)
{
    rtk_api_ret_t       retVal;
    rtk_uint32          phyData0;
    rtk_uint32          phyData4;
    ...
    if (rtk_switch_isComboPort(phydev, port) == RT_ERR_OK)
    {
        ...
        if(media_type == PORT_MEDIA_FIBER)
        {
            return _rtk_port_FiberModeAbility_get(phydev, port, pAbility);
        }
    }

    /*Control register setting and restart auto*/
    if ((retVal = rtl8367c_getAsicPHYReg(phydev, rtk_switch_port_L2P_get(phydev, port), PHY_CONTROL_REG, &phyData0)) != RT_ERR_OK)
        return retVal;

    /*Auto-Negotiation control register setting*/
    if ((retVal = rtl8367c_getAsicPHYReg(phydev, rtk_switch_port_L2P_get(phydev, port), PHY_AN_ADVERTISEMENT_REG, &phyData4)) != RT_ERR_OK)
        return retVal;

    /*1000 BASE-T control register setting*/
    if ((retVal = rtl8367c_getAsicPHYReg(phydev, rtk_switch_port_L2P_get(phydev, port), PHY_1000_BASET_CONTROL_REG, &phyData9)) != RT_ERR_OK)
        return retVal;
 ...
 */
rtk_api_ret_t rtk_port_phyForceModeAbility_set(struct phy_device *phydev, rtk_port_t port, rtk_port_phy_ability_t *pAbility)
{
     rtk_api_ret_t      retVal;
     rtk_uint32         phyData;
     rtk_uint32         phyEnMsk0;
 ...
 *      Get the capablity of specified PHY.
 */
rtk_api_ret_t rtk_port_phyForceModeAbility_get(struct phy_device *phydev, rtk_port_t port, rtk_port_phy_ability_t *pAbility)
{
    rtk_api_ret_t       retVal;
    rtk_uint32          phyData0;
    rtk_uint32          phyData4;
 ...
 */
rtk_api_ret_t rtk_port_phyStatus_get(struct phy_device *phydev, rtk_port_t port, rtk_port_linkStatus_t *pLinkStatus, rtk_port_speed_t *pSpeed, rtk_port_duplex_t *pDuplex)
{
    rtk_api_ret_t retVal;
    rtk_uint32 phyData;
    ...
    if ((retVal = rtl8367c_getAsicPHYReg(phydev, rtk_switch_port_L2P_get(phydev, port), PHY_RESOLVED_REG, &phyData)) != RT_ERR_OK)
        return retVal;
 ...
 */
rtk_api_ret_t rtk_port_macForceLink_set(struct phy_device *phydev, rtk_port_t port, rtk_port_mac_ability_t *pPortability)
{
    rtk_api_ret_t retVal;
    ...
    if ((retVal = rtl8367c_getAsicPortForceLink(phydev, rtk_switch_port_L2P_get(phydev, port), &ability)) != RT_ERR_OK)
        return retVal;
    ...
    if ((retVal = rtl8367c_setAsicPortForceLink(phydev, rtk_switch_port_L2P_get(phydev, port), &ability)) != RT_ERR_OK)
        return retVal;
 ...
 */
rtk_api_ret_t rtk_port_macForceLink_get(struct phy_device *phydev, rtk_port_t port, rtk_port_mac_ability_t *pPortability)
{
    rtk_api_ret_t retVal;
    rtl8367c_port_ability_t ability;
 ...
 *      - MODE_EXT_100FX,
 */
rtk_api_ret_t rtk_port_macForceLinkExt_set(struct phy_device *phydev, rtk_port_t port, rtk_mode_ext_t mode, rtk_port_mac_ability_t *pPortability)
{
    rtk_api_ret_t retVal;
    rtl8367c_port_ability_t ability;
    rtk_uint32 ext_id;
    ...
    if(mode == MODE_EXT_DISABLE)
    {
        ...
        if ((retVal = rtl8367c_setAsicPortForceLinkExt(phydev, ext_id, &ability)) != RT_ERR_OK)
            return retVal;

        if ((retVal = rtl8367c_setAsicPortExtMode(phydev, ext_id, mode)) != RT_ERR_OK)
            return retVal;
    }
    else
    {
        ...
        if ((retVal = rtl8367c_getAsicPortForceLinkExt(phydev, ext_id, &ability)) != RT_ERR_OK)
            return retVal;
 ...
 */
rtk_api_ret_t rtk_port_macForceLinkExt_get(struct phy_device *phydev, rtk_port_t port, rtk_mode_ext_t *pMode, rtk_port_mac_ability_t *pPortability)
{
    rtk_api_ret_t retVal;
    rtl8367c_port_ability_t ability;
    rtk_uint32 ext_id;
    ...
    if ((retVal = rtl8367c_getAsicPortExtMode(phydev, ext_id, (rtk_uint32 *)pMode)) != RT_ERR_OK)
        return retVal;
 ...
 */
rtk_api_ret_t rtk_port_macStatus_get(struct phy_device *phydev, rtk_port_t port, rtk_port_mac_ability_t *pPortstatus)
{
    rtk_api_ret_t retVal;
    ...
    if ((retVal = rtl8367c_getAsicPortStatus(phydev, rtk_switch_port_L2P_get(phydev, port), &status)) != RT_ERR_OK)
        return retVal;
    ...
    if( (retVal = rtl8367c_getAsicRegBit(phydev, RTL8367C_REG_SDS_MISC, RTL8367C_CFG_MAC8_SEL_HSGMII_OFFSET, &hsgsel)) != RT_ERR_OK)
            return retVal;

    if( (rtk_switch_isHsgPort(phydev, port) == RT_ERR_OK) && (hsgsel == 1) )
 ...
 */
rtk_api_ret_t rtk_port_macLocalLoopbackEnable_set(struct phy_device *phydev, rtk_port_t port, rtk_enable_t enable)
{
    rtk_api_ret_t   retVal;
    rtk_uint32      data;
    ...
    if ((retVal = rtl8367c_setAsicPortLoopback(phydev, rtk_switch_port_L2P_get(phydev, port), enable)) != RT_ERR_OK)
        return retVal;

    if(rtk_switch_isUtpPort(phydev, port) == RT_ERR_OK)
    {
        if ((retVal = rtl8367c_getAsicPHYReg(phydev, rtk_switch_port_L2P_get(phydev, port), PHY_CONTROL_REG, &data)) != RT_ERR_OK)
            return retVal;
        ...
        if ((retVal = rtl8367c_setAsicPHYReg(phydev, rtk_switch_port_L2P_get(phydev, port), PHY_CONTROL_REG, data)) != RT_ERR_OK)
            return retVal;
    }
 ...
 *      None.
 */
rtk_api_ret_t rtk_port_macLocalLoopbackEnable_get(struct phy_device *phydev, rtk_port_t port, rtk_enable_t *pEnable)
{
    rtk_api_ret_t retVal;
    ...
    if ((retVal = rtl8367c_getAsicPortLoopback(phydev, rtk_switch_port_L2P_get(phydev, port), pEnable)) != RT_ERR_OK)
        return retVal;
 ...
 */
rtk_api_ret_t rtk_port_phyReg_set(struct phy_device *phydev, rtk_port_t port, rtk_port_phy_reg_t reg, rtk_port_phy_data_t regData)
{
    rtk_api_ret_t retVal;
    ...
    if ((retVal = rtl8367c_setAsicPHYReg(phydev, rtk_switch_port_L2P_get(phydev, port), reg, regData)) != RT_ERR_OK)
        return retVal;
 ...
 */
rtk_api_ret_t rtk_port_phyReg_get(struct phy_device *phydev, rtk_port_t port, rtk_port_phy_reg_t reg, rtk_port_phy_data_t *pData)
{
    rtk_api_ret_t retVal;
    ...
    if ((retVal = rtl8367c_getAsicPHYReg(phydev, rtk_switch_port_L2P_get(phydev, port), reg, pData)) != RT_ERR_OK) {
        return retVal;
	}
 ...
 *      - ENABLE (Backpressure)
 */
rtk_api_ret_t rtk_port_backpressureEnable_set(struct phy_device *phydev, rtk_port_t port, rtk_enable_t enable)
{
    rtk_api_ret_t retVal;
    ...
    if ((retVal = rtl8367c_setAsicPortJamMode(phydev, !enable)) != RT_ERR_OK)
        return retVal;
 ...
 *      - ENABLE (Backpressure)
 */
rtk_api_ret_t rtk_port_backpressureEnable_get(struct phy_device *phydev, rtk_port_t port, rtk_enable_t *pEnable)
{
    rtk_api_ret_t retVal;
    rtk_uint32 regData;
    ...
    if ((retVal = rtl8367c_getAsicPortJamMode(phydev, &regData)) != RT_ERR_OK)
        return retVal;
 ...
 *      - ENABLE
 */
rtk_api_ret_t rtk_port_adminEnable_set(struct phy_device *phydev, rtk_port_t port, rtk_enable_t enable)
{
    rtk_api_ret_t retVal;
    rtk_uint32      data;
    ...
    if ((retVal = rtk_port_phyReg_get(phydev, port, PHY_CONTROL_REG, &data)) != RT_ERR_OK)
        return retVal;
    ...
    if ((retVal = rtk_port_phyReg_set(phydev, port, PHY_CONTROL_REG, data)) != RT_ERR_OK)
        return retVal;
 ...
 *      - ENABLE
 */
rtk_api_ret_t rtk_port_adminEnable_get(struct phy_device *phydev, rtk_port_t port, rtk_enable_t *pEnable)
{
    rtk_api_ret_t retVal;
    rtk_uint32      data;
 ...
 */
rtk_api_ret_t rtk_port_isolation_set(struct phy_device *phydev, rtk_port_t port, rtk_portmask_t *pPortmask)
{
    rtk_api_ret_t retVal;
    rtk_uint32 pmask;
    ...
    if ((retVal = rtl8367c_setAsicPortIsolationPermittedPortmask(phydev, rtk_switch_port_L2P_get(phydev, port), pmask)) != RT_ERR_OK)
        return retVal;
 ...
 *      A port can only transmit packet to ports included in permitted portmask
 */
rtk_api_ret_t rtk_port_isolation_get(struct phy_device *phydev, rtk_port_t port, rtk_portmask_t *pPortmask)
{
    rtk_api_ret_t retVal;
    rtk_uint32 pmask;
    ...
    if ((retVal = rtl8367c_getAsicPortIsolationPermittedPortmask(phydev, rtk_switch_port_L2P_get(phydev, port), &pmask)) != RT_ERR_OK)
        return retVal;
 ...
 */
rtk_api_ret_t rtk_port_rgmiiDelayExt_set(struct phy_device *phydev, rtk_port_t port, rtk_data_t txDelay, rtk_data_t rxDelay)
{
    rtk_api_ret_t retVal;
    ...
    if ((retVal = rtl8367c_getAsicReg(phydev, regAddr, &regData)) != RT_ERR_OK)
        return retVal;

    regData = (regData & 0xFFF0) | ((txDelay << 3) & 0x0008) | (rxDelay & 0x0007);

    if ((retVal = rtl8367c_setAsicReg(phydev, regAddr, regData)) != RT_ERR_OK)
        return retVal;
 ...
 */
rtk_api_ret_t rtk_port_rgmiiDelayExt_get(struct phy_device *phydev, rtk_port_t port, rtk_data_t *pTxDelay, rtk_data_t *pRxDelay)
{
    rtk_api_ret_t retVal;
    rtk_uint32 regAddr, regData;
 ...
 *      - ENABLE
 */
rtk_api_ret_t rtk_port_phyEnableAll_set(struct phy_device *phydev, rtk_enable_t enable)
{
    rtk_api_ret_t retVal;
    rtk_uint32 data;
    rtk_uint32 port;
    ...
    if ((retVal = rtl8367c_setAsicPortEnableAll(phydev, enable)) != RT_ERR_OK)
        return retVal;

    RTK_SCAN_ALL_LOG_PORT(phydev, port)
    {
        if(rtk_switch_isUtpPort(phydev, port) == RT_ERR_OK)
        {
            if ((retVal = rtk_port_phyReg_get(phydev, port, PHY_CONTROL_REG, &data)) != RT_ERR_OK)
 ...
 *      - ENABLE
 */
rtk_api_ret_t rtk_port_phyEnableAll_get(struct phy_device *phydev, rtk_enable_t *pEnable)
{
    rtk_api_ret_t retVal;
    ...
    if ((retVal = rtl8367c_getAsicPortEnableAll(phydev, pEnable)) != RT_ERR_OK)
        return retVal;
 ...
 */
rtk_api_ret_t rtk_port_efid_set(struct phy_device *phydev, rtk_port_t port, rtk_data_t efid)
{
    rtk_api_ret_t retVal;
    ...
    if ((retVal = rtl8367c_setAsicPortIsolationEfid(phydev, rtk_switch_port_L2P_get(phydev, port), efid))!=RT_ERR_OK)
        return retVal;
 ...
 */
rtk_api_ret_t rtk_port_efid_get(struct phy_device *phydev, rtk_port_t port, rtk_data_t *pEfid)
{
    rtk_api_ret_t retVal;
    ...
    if ((retVal = rtl8367c_getAsicPortIsolationEfid(phydev, rtk_switch_port_L2P_get(phydev, port), pEfid))!=RT_ERR_OK)
        return retVal;
 ...
 */
rtk_api_ret_t rtk_port_phyComboPortMedia_set(struct phy_device *phydev, rtk_port_t port, rtk_port_media_t media)
{
    rtk_api_ret_t retVal;
    rtk_uint32 regData;
    rtk_uint32 idx;
    ...
    {
        /* software init */
        if ((retVal = rtl8367c_setAsicRegBit(phydev, RTL8367C_REG_CHIP_RESET, RTL8367C_DW8051_RST_OFFSET, 1)) != RT_ERR_OK)
            return retVal;

        if ((retVal = rtl8367c_setAsicRegBit(phydev, RTL8367C_REG_MISCELLANEOUS_CONFIGURE0, RTL8367C_DW8051_EN_OFFSET, 1)) != RT_ERR_OK)
            return retVal;

        if ((retVal = rtl8367c_setAsicRegBit(phydev, RTL8367C_REG_DW8051_RDY, RTL8367C_ACS_IROM_ENABLE_OFFSET, 1)) != RT_ERR_OK)
            return retVal;

        if ((retVal = rtl8367c_setAsicRegBit(phydev, RTL8367C_REG_DW8051_RDY, RTL8367C_IROM_MSB_OFFSET, 0)) != RT_ERR_OK)
            return retVal;

        for(idx = 0; idx < FIBER_INIT_SIZE; idx++)
        {
            if ((retVal = rtl8367c_setAsicReg(phydev, 0xE000 + idx, (rtk_uint32)Fiber[idx])) != RT_ERR_OK)
                return retVal;
        }
        ...
        if ((retVal = rtl8367c_setAsicRegBit(phydev, RTL8367C_REG_DW8051_RDY, RTL8367C_ACS_IROM_ENABLE_OFFSET, 0)) != RT_ERR_OK)
            return retVal;

        if ((retVal = rtl8367c_setAsicRegBit(phydev, RTL8367C_REG_CHIP_RESET, RTL8367C_DW8051_RST_OFFSET, 0)) != RT_ERR_OK)
            return retVal;
    }
    else
    {
        if ((retVal = rtl8367c_setAsicRegBit(phydev, RTL8367C_REG_UTP_FIB_DET, RTL8367C_UTP_FIRST_OFFSET, 1))!=RT_ERR_OK)
            return retVal;

        if ((retVal = rtl8367c_setAsicRegBit(phydev, RTL8367C_REG_DW8051_RDY, RTL8367C_DW8051_READY_OFFSET, 0)) != RT_ERR_OK)
            return retVal;
    }
 ...
 *      The API can Set Combo port media type.
 */
rtk_api_ret_t rtk_port_phyComboPortMedia_get(struct phy_device *phydev, rtk_port_t port, rtk_port_media_t *pMedia)
{
    rtk_api_ret_t   retVal;
    rtk_uint32      regData;
    rtk_uint32      data;
    ...
    {
        if ((retVal = rtl8367c_getAsicRegBit(phydev, RTL8367C_REG_UTP_FIB_DET, RTL8367C_UTP_FIRST_OFFSET, &data))!=RT_ERR_OK)
                return retVal;
 ...
 */
rtk_api_ret_t rtk_port_rtctEnable_set(struct phy_device *phydev, rtk_portmask_t *pPortmask)
{
    rtk_api_ret_t   retVal;
    ...
    RTK_CHK_PORTMASK_VALID_ONLY_UTP(phydev, pPortmask);

    if ((retVal = rtl8367c_setAsicPortRTCTEnable(phydev, pPortmask->bits[0]))!=RT_ERR_OK)
        return retVal;
 ...
 */
rtk_api_ret_t rtk_port_rtctDisable_set(struct phy_device *phydev, rtk_portmask_t *pPortmask)
{
    rtk_api_ret_t   retVal;
    ...
    if ((retVal = rtl8367c_setAsicPortRTCTDisable(phydev, pPortmask->bits[0]))!=RT_ERR_OK)
        return retVal;
 ...
 */
rtk_api_ret_t rtk_port_rtctResult_get(struct phy_device *phydev, rtk_port_t port, rtk_rtctResult_t *pRtctResult)
{
    rtk_api_ret_t               retVal;
    ...
    if ((retVal = rtl8367c_getAsicPortRTCTResult(phydev, port, &result))!=RT_ERR_OK)
        return retVal;
 ...
 */
rtk_api_ret_t rtk_port_sds_reset(struct phy_device *phydev, rtk_port_t port)
{
    rtk_uint32 ext_id;
    ...
    /* Check Port Valid */
    if(rtk_switch_isSgmiiPort(phydev, port) != RT_ERR_OK)
        return RT_ERR_PORT_ID;

    ext_id = port - 15;
    return rtl8367c_sdsReset(phydev, ext_id);
}
 ...
 *      The API can reset Serdes
 */
rtk_api_ret_t rtk_port_sgmiiLinkStatus_get(struct phy_device *phydev, rtk_port_t port, rtk_data_t *pSignalDetect, rtk_data_t *pSync, rtk_port_linkStatus_t *pLink)
{
    rtk_uint32 ext_id;
    ...
    ext_id = port - 15;
    return rtl8367c_getSdsLinkStatus(phydev, ext_id, (rtk_uint32 *)pSignalDetect, (rtk_uint32 *)pSync, (rtk_uint32 *)pLink);
}
 ...
 */
rtk_api_ret_t rtk_port_sgmiiNway_set(struct phy_device *phydev, rtk_port_t port, rtk_enable_t state)
{
    rtk_uint32 ext_id;
    ...
    ext_id = port - 15;
    return rtl8367c_setSgmiiNway(phydev, ext_id, (rtk_uint32)state);
}
 ...
 */
rtk_api_ret_t rtk_port_sgmiiNway_get(struct phy_device *phydev, rtk_port_t port, rtk_enable_t *pState)
{
    rtk_uint32 ext_id;
    ...
    ext_id = port - 15;
    return rtl8367c_getSgmiiNway(phydev, ext_id, (rtk_uint32 *)pState);
}
