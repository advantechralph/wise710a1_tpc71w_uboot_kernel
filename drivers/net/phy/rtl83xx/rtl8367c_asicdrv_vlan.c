 ...
 *      None
 */
ret_t rtl8367c_setAsicVlanMemberConfig(struct phy_device *phydev, rtk_uint32 index, rtl8367c_vlanconfiguser *pVlanCg)
{
    ret_t  retVal;
    rtk_uint32 regAddr;
    rtk_uint32 regData;
 ...
 *      None
 */
ret_t rtl8367c_getAsicVlanMemberConfig(struct phy_device *phydev, rtk_uint32 index, rtl8367c_vlanconfiguser *pVlanCg)
{
    ret_t  retVal;
    rtk_uint32 page_idx;
    rtk_uint32 regAddr;
 ...
 *      None
 */
ret_t rtl8367c_setAsicVlan4kEntry(struct phy_device *phydev, rtl8367c_user_vlan4kentry *pVlan4kEntry )
{
    ...
    {
        regData = *tableAddr;
        retVal = rtl8367c_setAsicReg(phydev, RTL8367C_TABLE_ACCESS_WRDATA_BASE + page_idx, regData);
        if(retVal != RT_ERR_OK)
            return retVal;
    ...
    retVal = rtl8367c_setAsicReg(phydev, RTL8367C_TABLE_ACCESS_ADDR_REG, regData);
    if(retVal != RT_ERR_OK)
        return retVal;

    /* Write Command */
    retVal = rtl8367c_setAsicRegBits(phydev, RTL8367C_TABLE_ACCESS_CTRL_REG, RTL8367C_TABLE_TYPE_MASK | RTL8367C_COMMAND_TYPE_MASK,RTL8367C_TABLE_ACCESS_REG_DATA(TB_OP_WRITE,TB_TARGET_CVLAN));
    if(retVal != RT_ERR_OK)
        return retVal;
 ...
 *      None
 */
ret_t rtl8367c_getAsicVlan4kEntry(struct phy_device *phydev, rtl8367c_user_vlan4kentry *pVlan4kEntry )
{
    rtk_uint16                  vlan_4k_entry[RTL8367C_VLAN_4KTABLE_LEN];
    rtk_uint32                  page_idx;
    rtk_uint16                  *tableAddr;
    ...
    /* Read Command */
    retVal = rtl8367c_setAsicRegBits(phydev, RTL8367C_TABLE_ACCESS_CTRL_REG, RTL8367C_TABLE_TYPE_MASK | RTL8367C_COMMAND_TYPE_MASK, RTL8367C_TABLE_ACCESS_REG_DATA(TB_OP_READ,TB_TARGET_CVLAN));
    if(retVal != RT_ERR_OK)
        return retVal;
    ...
    for(page_idx = 0; page_idx < RTL8367C_VLAN_4KTABLE_LEN; page_idx++)
    {
        retVal = rtl8367c_getAsicReg(phydev, RTL8367C_TABLE_ACCESS_RDDATA_BASE + page_idx, &regData);
        if(retVal != RT_ERR_OK)
            return retVal;
 ...
 *      None
 */
ret_t rtl8367c_setAsicVlanAccpetFrameType(struct phy_device *phydev, rtk_uint32 port, rtl8367c_accframetype frameType)
{
    ...
    return rtl8367c_setAsicRegBits(phydev, RTL8367C_VLAN_ACCEPT_FRAME_TYPE_REG(port), RTL8367C_VLAN_ACCEPT_FRAME_TYPE_MASK(port), frameType);
}
/* Function Name:
 ...
 *      None
 */
ret_t rtl8367c_getAsicVlanAccpetFrameType(struct phy_device *phydev, rtk_uint32 port, rtl8367c_accframetype *pFrameType)
{
    rtk_uint32 regData;
    ret_t  retVal;
    ...
    if((retVal = rtl8367c_getAsicRegBits(phydev, RTL8367C_VLAN_ACCEPT_FRAME_TYPE_REG(port), RTL8367C_VLAN_ACCEPT_FRAME_TYPE_MASK(port), &regData)) != RT_ERR_OK)
        return retVal;
 ...
 *      None
 */
ret_t rtl8367c_setAsicVlanIngressFilter(struct phy_device *phydev, rtk_uint32 port, rtk_uint32 enabled)
{
    if(port > RTL8367C_PORTIDMAX)
        return RT_ERR_PORT_ID;

    return rtl8367c_setAsicRegBit(phydev, RTL8367C_VLAN_INGRESS_REG, port, enabled);
}
/* Function Name:
 ...
 *      None
 */
ret_t rtl8367c_getAsicVlanIngressFilter(struct phy_device *phydev, rtk_uint32 port, rtk_uint32 *pEnable)
{
    if(port > RTL8367C_PORTIDMAX)
        return RT_ERR_PORT_ID;

    return rtl8367c_getAsicRegBit(phydev, RTL8367C_VLAN_INGRESS_REG, port, pEnable);
}
/* Function Name:
 ...
 *      None
 */
ret_t rtl8367c_setAsicVlanEgressTagMode(struct phy_device *phydev, rtk_uint32 port, rtl8367c_egtagmode tagMode)
{
    if(port > RTL8367C_PORTIDMAX)
        return RT_ERR_PORT_ID;
    ...
    return rtl8367c_setAsicRegBits(phydev, RTL8367C_PORT_MISC_CFG_REG(port), RTL8367C_VLAN_EGRESS_MDOE_MASK, tagMode);
}
/* Function Name:
 ...
 *      None
 */
ret_t rtl8367c_getAsicVlanEgressTagMode(struct phy_device *phydev, rtk_uint32 port, rtl8367c_egtagmode *pTagMode)
{
    rtk_uint32 regData;
    ret_t  retVal;
    ...
    if((retVal = rtl8367c_getAsicRegBits(phydev, RTL8367C_PORT_MISC_CFG_REG(port), RTL8367C_VLAN_EGRESS_MDOE_MASK, &regData)) != RT_ERR_OK)
        return retVal;
 ...
 *      None
 */
ret_t rtl8367c_setAsicVlanPortBasedVID(struct phy_device *phydev, rtk_uint32 port, rtk_uint32 index, rtk_uint32 pri)
{
    ...
    retVal = rtl8367c_setAsicRegBits(phydev, regAddr, bit_mask, index);
    if(retVal != RT_ERR_OK)
        return retVal;

    regAddr = RTL8367C_VLAN_PORTBASED_PRIORITY_REG(port);
    bit_mask = RTL8367C_VLAN_PORTBASED_PRIORITY_MASK(port);
    retVal = rtl8367c_setAsicRegBits(phydev, regAddr, bit_mask, pri);
    if(retVal != RT_ERR_OK)
        return retVal;
 ...
 *      None
 */
ret_t rtl8367c_getAsicVlanPortBasedVID(struct phy_device *phydev, rtk_uint32 port, rtk_uint32 *pIndex, rtk_uint32 *pPri)
{
    rtk_uint32 regAddr,bit_mask;
    ret_t  retVal;
    ...
    regAddr = RTL8367C_VLAN_PVID_CTRL_REG(port);
    bit_mask = RTL8367C_PORT_VIDX_MASK(port);
    retVal = rtl8367c_getAsicRegBits(phydev, regAddr, bit_mask, pIndex);
    if(retVal != RT_ERR_OK)
        return retVal;

    regAddr = RTL8367C_VLAN_PORTBASED_PRIORITY_REG(port);
    bit_mask = RTL8367C_VLAN_PORTBASED_PRIORITY_MASK(port);
    retVal = rtl8367c_getAsicRegBits(phydev, regAddr, bit_mask, pPri);
    if(retVal != RT_ERR_OK)
        return retVal;
 ...
 *      None
 */
ret_t rtl8367c_setAsicVlanProtocolBasedGroupData(struct phy_device *phydev, rtk_uint32 index, rtl8367c_protocolgdatacfg *pPbCfg)
{
    ...
    /* Frame type */
    retVal = rtl8367c_setAsicRegBits(phydev, RTL8367C_VLAN_PPB_FRAMETYPE_REG(index), RTL8367C_VLAN_PPB_FRAMETYPE_MASK, frameType);
    if(retVal != RT_ERR_OK)
        return retVal;

    /* Ether type */
    retVal = rtl8367c_setAsicReg(phydev, RTL8367C_VLAN_PPB_ETHERTYPR_REG(index), etherType);
    if(retVal != RT_ERR_OK)
        return retVal;
 ...
 *      None
 */
ret_t rtl8367c_getAsicVlanProtocolBasedGroupData(struct phy_device *phydev, rtk_uint32 index, rtl8367c_protocolgdatacfg *pPbCfg)
{
    rtk_uint32  frameType;
    rtk_uint32  etherType;
    ret_t   retVal;
    ...
    /* Read Frame type */
    retVal = rtl8367c_getAsicRegBits(phydev, RTL8367C_VLAN_PPB_FRAMETYPE_REG(index), RTL8367C_VLAN_PPB_FRAMETYPE_MASK, &frameType);
    if(retVal != RT_ERR_OK)
        return retVal;

    /* Read Ether type */
    retVal = rtl8367c_getAsicReg(phydev, RTL8367C_VLAN_PPB_ETHERTYPR_REG(index), &etherType);
    if(retVal != RT_ERR_OK)
        return retVal;
 ...
 *      None
 */
ret_t rtl8367c_setAsicVlanPortAndProtocolBased(struct phy_device *phydev, rtk_uint32 port, rtk_uint32 index, rtl8367c_protocolvlancfg *pPpbCfg)
{
    ...
    retVal    = rtl8367c_setAsicRegBits(phydev, reg_addr, bit_mask, bit_value);
    if(retVal != RT_ERR_OK)
        return retVal;
 ...
 *      None
 */
ret_t rtl8367c_getAsicVlanPortAndProtocolBased(struct phy_device *phydev, rtk_uint32 port, rtk_uint32 index, rtl8367c_protocolvlancfg *pPpbCfg)
{
    rtk_uint32  reg_addr, bit_mask, bit_value;
    ret_t   retVal;
    ...
    reg_addr  = RTL8367C_VLAN_PPB_VALID_REG(index);
    bit_mask  = 0x0001 << port;
    retVal    = rtl8367c_getAsicRegBits(phydev, reg_addr, bit_mask, &bit_value);
    if(retVal != RT_ERR_OK)
        return retVal;
 ...
 *      None
 */
ret_t rtl8367c_setAsicVlanFilter(struct phy_device *phydev, rtk_uint32 enabled)
{
    return rtl8367c_setAsicRegBit(phydev, RTL8367C_REG_VLAN_CTRL, RTL8367C_VLAN_CTRL_OFFSET, enabled);
}
/* Function Name:
 ...
 *      None
 */
ret_t rtl8367c_getAsicVlanFilter(struct phy_device *phydev, rtk_uint32* pEnabled)
{
    return rtl8367c_getAsicRegBit(phydev, RTL8367C_REG_VLAN_CTRL, RTL8367C_VLAN_CTRL_OFFSET, pEnabled);
}
/* Function Name:
 ...
 *      None
 */
ret_t rtl8367c_setAsicVlanUntagDscpPriorityEn(struct phy_device *phydev, rtk_uint32 enabled)
{
    return rtl8367c_setAsicRegBit(phydev, RTL8367C_REG_UNTAG_DSCP_PRI_CFG, RTL8367C_UNTAG_DSCP_PRI_CFG_OFFSET, enabled);
}
/* Function Name:
 ...
 *      None
 */
ret_t rtl8367c_getAsicVlanUntagDscpPriorityEn(struct phy_device *phydev, rtk_uint32* enabled)
{
    return rtl8367c_getAsicRegBit(phydev, RTL8367C_REG_UNTAG_DSCP_PRI_CFG, RTL8367C_UNTAG_DSCP_PRI_CFG_OFFSET, enabled);
}
/* Function Name:
 ...
 *      None
 */
ret_t rtl8367c_setAsicPortBasedFid(struct phy_device *phydev, rtk_uint32 port, rtk_uint32 fid)
{
    rtk_uint32  reg_addr;
    ...
    if(port < 8)
        return rtl8367c_setAsicReg(phydev, RTL8367C_PORT_PBFID_REG(port),fid);
    else {
        reg_addr = RTL8367C_REG_PORT8_PBFID + port-8;
        return rtl8367c_setAsicReg(phydev, reg_addr, fid);
    }
 ...
 *      None
 */
ret_t rtl8367c_getAsicPortBasedFid(struct phy_device *phydev, rtk_uint32 port, rtk_uint32* pFid)
{
    rtk_uint32  reg_addr;
    ...
    if(port < 8)
        return rtl8367c_getAsicReg(phydev, RTL8367C_PORT_PBFID_REG(port), pFid);
    else{
        reg_addr = RTL8367C_REG_PORT8_PBFID + port-8;
        return rtl8367c_getAsicReg(phydev, reg_addr, pFid);
    }
}
/* Function Name:
 ...
 *      None
 */
ret_t rtl8367c_setAsicPortBasedFidEn(struct phy_device *phydev, rtk_uint32 port, rtk_uint32 enabled)
{
    if(port > RTL8367C_PORTIDMAX)
        return RT_ERR_PORT_ID;

    return rtl8367c_setAsicRegBit(phydev, RTL8367C_REG_PORT_PBFIDEN,port, enabled);
}
/* Function Name:
 ...
 *      None
 */
ret_t rtl8367c_getAsicPortBasedFidEn(struct phy_device *phydev, rtk_uint32 port, rtk_uint32* pEnabled)
{
    if(port > RTL8367C_PORTIDMAX)
        return RT_ERR_PORT_ID;

    return rtl8367c_getAsicRegBit(phydev, RTL8367C_REG_PORT_PBFIDEN,port, pEnabled);
}
/* Function Name:
 ...
 *      None
 */
ret_t rtl8367c_setAsicSpanningTreeStatus(struct phy_device *phydev, rtk_uint32 port, rtk_uint32 msti, rtk_uint32 state)
{
    ...
    if(port < 8)
        return rtl8367c_setAsicRegBits(phydev, RTL8367C_VLAN_MSTI_REG(msti,port), RTL8367C_VLAN_MSTI_MASK(port),state);
    else{
        ...
        return rtl8367c_setAsicRegBits(phydev, reg_addr, bits_msk,state);
    }
}
/* Function Name:
 ...
 *      None
 */
ret_t rtl8367c_getAsicSpanningTreeStatus(struct phy_device *phydev, rtk_uint32 port, rtk_uint32 msti, rtk_uint32* pState)
{
    rtk_uint32  reg_addr,bits_msk;
    ...
    if(port < 8)
        return rtl8367c_getAsicRegBits(phydev, RTL8367C_VLAN_MSTI_REG(msti,port), RTL8367C_VLAN_MSTI_MASK(port), pState);
    else{
        reg_addr = RTL8367C_VLAN_MSTI_REG(msti,port);
        switch(port){
            case 8: bits_msk = RTL8367C_VLAN_MSTI0_CTRL1_PORT8_STATE_MASK;break;
        ...
        return rtl8367c_getAsicRegBits(phydev, reg_addr, bits_msk, pState);
    }
 ...
 *      None
 */
ret_t rtl8367c_setAsicVlanTransparent(struct phy_device *phydev, rtk_uint32 port, rtk_uint32 portmask)
{
    if(port > RTL8367C_PORTIDMAX)
        return RT_ERR_PORT_ID;
    ...
    return rtl8367c_setAsicRegBits(phydev, RTL8367C_REG_VLAN_EGRESS_TRANS_CTRL0 + port, RTL8367C_VLAN_EGRESS_TRANS_CTRL0_MASK, portmask);
}
 ...
 *      None
 */
ret_t rtl8367c_getAsicVlanTransparent(struct phy_device *phydev, rtk_uint32 port, rtk_uint32 *pPortmask)
{
    if(port > RTL8367C_PORTIDMAX)
        return RT_ERR_PORT_ID;

    return rtl8367c_getAsicRegBits(phydev, RTL8367C_REG_VLAN_EGRESS_TRANS_CTRL0 + port, RTL8367C_VLAN_EGRESS_TRANS_CTRL0_MASK, pPortmask);
}
 ...
 *      None
 */
ret_t rtl8367c_setAsicVlanEgressKeep(struct phy_device *phydev, rtk_uint32 port, rtk_uint32 portmask)
{
    rtk_uint32 regAddr, bit_mask;
    ret_t  retVal;
    ...
    if(port < 8){
        retVal = rtl8367c_setAsicRegBits(phydev, RTL8367C_REG_VLAN_EGRESS_KEEP_CTRL0 + (port>>1),RTL8367C_PORT0_VLAN_KEEP_MASK_MASK<<((port&1)*8),portmask & 0xff);
        if(retVal != RT_ERR_OK)
            return retVal;
        regAddr = RTL8367C_REG_VLAN_EGRESS_KEEP_CTRL0_EXT + (port>>1);
        bit_mask = RTL8367C_PORT0_VLAN_KEEP_MASK_EXT_MASK;
        bit_mask <<= (port&1)*3;
        retVal = rtl8367c_setAsicRegBits(phydev, regAddr, bit_mask, (portmask>>8)&0x7);
        if(retVal != RT_ERR_OK)
            return retVal;
    }
    else{
        switch(port){
            case 8:
                ...
                retVal = rtl8367c_setAsicRegBits(phydev, regAddr, bit_mask, portmask & 0xff);
                if(retVal != RT_ERR_OK)
                    return retVal;
 ...
 *      None
 */
ret_t rtl8367c_getAsicVlanEgressKeep(struct phy_device *phydev, rtk_uint32 port, rtk_uint32* pPortmask)
{
    ...
    if(port < 8){
        retVal = rtl8367c_getAsicRegBits(phydev, RTL8367C_REG_VLAN_EGRESS_KEEP_CTRL0 + (port>>1),RTL8367C_PORT0_VLAN_KEEP_MASK_MASK<<((port&1)*8),&regval_l);
        if(retVal != RT_ERR_OK)
            return retVal;
        regAddr = RTL8367C_REG_VLAN_EGRESS_KEEP_CTRL0_EXT + (port>>1);
        bit_mask = RTL8367C_PORT0_VLAN_KEEP_MASK_EXT_MASK;
        bit_mask <<= (port&1)*3;
        retVal = rtl8367c_getAsicRegBits(phydev, regAddr, bit_mask, &regval_h);
        if(retVal != RT_ERR_OK)
            return retVal;
        *pPortmask = (regval_h << 8) | regval_l;
        ...
        switch(port){
            case 8:
                regAddr = RTL8367C_REG_VLAN_EGRESS_KEEP_CTRL4;
                bit_mask = RTL8367C_PORT8_VLAN_KEEP_MASK_MASK;
                retVal = rtl8367c_getAsicRegBits(phydev, regAddr, bit_mask, &regval_l);
                if(retVal != RT_ERR_OK)
                    return retVal;
                regAddr = RTL8367C_REG_VLAN_EGRESS_KEEP_CTRL4_EXT;
                bit_mask = RTL8367C_PORT8_VLAN_KEEP_MASK_EXT_MASK;
 ...
 *      None
 */
ret_t rtl8367c_setReservedVidAction(struct phy_device *phydev, rtk_uint32 vid0Action, rtk_uint32 vid4095Action)
{
    ret_t   retVal;
    ...
    if((retVal = rtl8367c_setAsicRegBit(phydev, RTL8367C_REG_VLAN_EXT_CTRL, RTL8367C_VLAN_VID0_TYPE_OFFSET, vid0Action)) != RT_ERR_OK)
        return retVal;

    if((retVal = rtl8367c_setAsicRegBit(phydev, RTL8367C_REG_VLAN_EXT_CTRL, RTL8367C_VLAN_VID4095_TYPE_OFFSET, vid4095Action)) != RT_ERR_OK)
        return retVal;
 ...
 *      None
 */
ret_t rtl8367c_getReservedVidAction(struct phy_device *phydev, rtk_uint32 *pVid0Action, rtk_uint32 *pVid4095Action)
{
    ret_t   retVal;
    ...
    if((retVal = rtl8367c_getAsicRegBit(phydev, RTL8367C_REG_VLAN_EXT_CTRL, RTL8367C_VLAN_VID0_TYPE_OFFSET, pVid0Action)) != RT_ERR_OK)
        return retVal;

    if((retVal = rtl8367c_getAsicRegBit(phydev, RTL8367C_REG_VLAN_EXT_CTRL, RTL8367C_VLAN_VID4095_TYPE_OFFSET, pVid4095Action)) != RT_ERR_OK)
        return retVal;
 ...
 *      None
 */
ret_t rtl8367c_setRealKeepRemarkEn(struct phy_device *phydev, rtk_uint32 enabled)
{
    ret_t   retVal;

    if((retVal = rtl8367c_setAsicRegBit(phydev, RTL8367C_REG_VLAN_EXT_CTRL, RTL8367C_VLAN_1P_REMARK_BYPASS_REALKEEP_OFFSET, enabled)) != RT_ERR_OK)
        return retVal;
 ...
 *      None
 */
ret_t rtl8367c_getRealKeepRemarkEn(struct phy_device *phydev, rtk_uint32 *pEnabled)
{
    ret_t   retVal;

    if((retVal = rtl8367c_getAsicRegBit(phydev, RTL8367C_REG_VLAN_EXT_CTRL, RTL8367C_VLAN_1P_REMARK_BYPASS_REALKEEP_OFFSET, pEnabled)) != RT_ERR_OK)
        return retVal;
 ...
 *      None
 */
ret_t rtl8367c_resetVlan(struct phy_device *phydev)
{
    ret_t   retVal;

    if((retVal = rtl8367c_setAsicRegBit(phydev, RTL8367C_REG_VLAN_EXT_CTRL2, RTL8367C_VLAN_EXT_CTRL2_OFFSET, 1)) != RT_ERR_OK)
        return retVal;
