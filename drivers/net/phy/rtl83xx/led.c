 ...
 */
rtk_api_ret_t rtk_led_enable_set(struct phy_device *phydev, rtk_led_group_t group, rtk_portmask_t *pPortmask)
{
    ...
    {
        if(rtk_switch_isCPUPort(phydev, port) == RT_ERR_OK)
    ...
    if ((retVal = rtl8367c_setAsicLedGroupEnable(phydev, group, pmask)) != RT_ERR_OK)
        return retVal;
 ...
 */
rtk_api_ret_t rtk_led_enable_get(struct phy_device *phydev, rtk_led_group_t group, rtk_portmask_t *pPortmask)
{
    rtk_api_ret_t retVal;
    rtk_uint32 pmask;
    ...
    if ((retVal = rtl8367c_getAsicLedGroupEnable(phydev, group, &pmask)) != RT_ERR_OK)
        return retVal;
 ...
 *      - LED_OP_SERIAL,
 */
rtk_api_ret_t rtk_led_operation_set(struct phy_device *phydev, rtk_led_operation_t mode)
{
    rtk_api_ret_t retVal;
    rtk_uint32 regData;
    ...
    if ((retVal = rtl8367c_setAsicLedOperationMode(phydev, regData)) != RT_ERR_OK)
        return retVal;
 ...
 *      - LED_OP_SERIAL,
 */
rtk_api_ret_t rtk_led_operation_get(struct phy_device *phydev, rtk_led_operation_t *pMode)
{
    rtk_api_ret_t retVal;
    rtk_uint32 regData;
    ...
    if ((retVal = rtl8367c_getAsicLedOperationMode(phydev, &regData)) != RT_ERR_OK)
        return retVal;
 ...
 *      - LED_FORCE_ON.
 */
rtk_api_ret_t rtk_led_modeForce_set(struct phy_device *phydev, rtk_port_t port, rtk_led_group_t group, rtk_led_force_mode_t mode)
{
    rtk_api_ret_t retVal;
    ...
    if ((retVal = rtl8367c_setAsicForceLed(phydev, rtk_switch_port_L2P_get(phydev, port), group, mode)) != RT_ERR_OK)
        return retVal;
 ...
 *      - LED_FORCE_ON.
 */
rtk_api_ret_t rtk_led_modeForce_get(struct phy_device *phydev, rtk_port_t port, rtk_led_group_t group, rtk_led_force_mode_t *pMode)
{
    rtk_api_ret_t retVal;
    ...
    if ((retVal = rtl8367c_getAsicForceLed(phydev, rtk_switch_port_L2P_get(phydev, port), group, pMode)) != RT_ERR_OK)
        return retVal;
 ...
 */
rtk_api_ret_t rtk_led_blinkRate_set(struct phy_device *phydev, rtk_led_blink_rate_t blinkRate)
{
    rtk_api_ret_t retVal;
    ...
    if ((retVal = rtl8367c_setAsicLedBlinkRate(phydev, blinkRate)) != RT_ERR_OK)
        return retVal;
 ...
 */
rtk_api_ret_t rtk_led_blinkRate_get(struct phy_device *phydev, rtk_led_blink_rate_t *pBlinkRate)
{
    rtk_api_ret_t retVal;
    ...
    if ((retVal = rtl8367c_getAsicLedBlinkRate(phydev, pBlinkRate)) != RT_ERR_OK)
        return retVal;
 ...
 */
rtk_api_ret_t rtk_led_groupConfig_set(struct phy_device *phydev, rtk_led_group_t group, rtk_led_congig_t config)
{
    rtk_api_ret_t retVal;
    ...
    if ((retVal = rtl8367c_setAsicLedIndicateInfoConfig(phydev, group, config)) != RT_ERR_OK)
        return retVal;
 ...
 */
rtk_api_ret_t rtk_led_groupConfig_get(struct phy_device *phydev, rtk_led_group_t group, rtk_led_congig_t *pConfig)
{
    rtk_api_ret_t retVal;
    ...
    if ((retVal = rtl8367c_getAsicLedIndicateInfoConfig(phydev, group, pConfig)) != RT_ERR_OK)
        return retVal;
...
rtk_api_ret_t rtk_led_groupAbility_set(struct phy_device *phydev, rtk_led_group_t group, rtk_led_ability_t *pAbility)
{
    rtk_api_ret_t retVal;
    rtk_uint32 regData;
    ...
    if ((retVal = rtl8367c_getAsicReg(phydev, RTL8367C_REG_LED0_DATA_CTRL + (rtk_uint32)group, &regData)) != RT_ERR_OK)
        return retVal;
    ...
    if ((retVal = rtl8367c_setAsicReg(phydev, RTL8367C_REG_LED0_DATA_CTRL + (rtk_uint32)group, regData)) != RT_ERR_OK)
        return retVal;
...
rtk_api_ret_t rtk_led_groupAbility_get(struct phy_device *phydev, rtk_led_group_t group, rtk_led_ability_t *pAbility)
{
    rtk_api_ret_t retVal;
    rtk_uint32 regData;
 ...
 */
rtk_api_ret_t rtk_led_serialMode_set(struct phy_device *phydev, rtk_led_active_t active)
{
    rtk_api_ret_t retVal;
    ...
    if ( active >= LED_ACTIVE_END)
     ...
     if ((retVal = rtl8367c_setAsicLedSerialModeConfig(phydev, active,1))!=RT_ERR_OK)
        return retVal;
 ...
 */
rtk_api_ret_t rtk_led_serialMode_get(struct phy_device *phydev, rtk_led_active_t *pActive)
{
    rtk_api_ret_t retVal;
    rtk_uint32 regData;
    ...
    if ((retVal = rtl8367c_getAsicLedSerialModeConfig(phydev, pActive,&regData))!=RT_ERR_OK)
        return retVal;
 ...
 *      This API set LED I/O state.
 */
rtk_api_ret_t rtk_led_OutputEnable_set(struct phy_device *phydev, rtk_enable_t state)
{
    rtk_api_ret_t retVal;
    ...
    if ((retVal = rtl8367c_setAsicLedOutputEnable(phydev, state))!=RT_ERR_OK)
        return retVal;
 ...
 */
rtk_api_ret_t rtk_led_OutputEnable_get(struct phy_device *phydev, rtk_enable_t *pState)
{
    rtk_api_ret_t retVal;
    ...
    if ((retVal = rtl8367c_getAsicLedOutputEnable(phydev, pState))!=RT_ERR_OK)
        return retVal;
 ...
 *      None.
 */
rtk_api_ret_t rtk_led_serialModePortmask_set(struct phy_device *phydev, rtk_led_serialOutput_t output, rtk_portmask_t *pPortmask)
{
    rtk_api_ret_t retVal;
    rtk_uint32 pmask;
    ...
    if ((retVal = rtl8367c_setAsicLedSerialOutput(phydev, (rtk_uint32)output, pmask))!=RT_ERR_OK)
        return retVal;
 ...
 *      None.
 */
rtk_api_ret_t rtk_led_serialModePortmask_get(struct phy_device *phydev, rtk_led_serialOutput_t *pOutput, rtk_portmask_t *pPortmask)
{
    rtk_api_ret_t retVal;
    rtk_uint32 pmask;
    ...
    if ((retVal = rtl8367c_getAsicLedSerialOutput(phydev, (rtk_uint32 *)pOutput, &pmask))!=RT_ERR_OK)
        return retVal;
