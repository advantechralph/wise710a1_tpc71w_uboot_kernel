/*
 * advat24.c - handle AT24C08 I2C EEPROM for Advantech TPC-71W.
 *
 * Copyright (C) 2005-2007 David Brownell
 * Copyright (C) 2008 Wolfram Sang, Pengutronix
 * Copyright (C) 2018 Ji Xu, Advantech
 *
...
#define ADVANTECH_EC_EEPROM_VER           "1.00"
#define ADVANTECH_EC_EEPROM_DATE          "01/21/2019" 

#define USEFOR_ADVANTECH_AT24C08
#ifdef USEFOR_ADVANTECH_AT24C08
...
#endif
#endif // USEFOR_ADVANTECH_AT24C08
...
static struct i2c_driver advat24_driver = {
	.driver = {
		.name = "at24",
		.owner = THIS_MODULE,
	},
...
static int advat24_eeprom_ioctl(struct inode *inode, struct file *filp, unsigned int cmd, unsigned long arg )
#else
static long advat24_eeprom_ioctl(struct file* filp, unsigned int cmd, unsigned long arg )
#endif
{
	...
	case IOCTL_COMMON_EC_I2C_TRANSFER:
	{
		...
		if (copy_from_user(&EcI2cData, (ec_i2c_data __user *)arg, sizeof(ec_i2c_data))) {
			...
			// 5. Start Transfter
				// Retrieve an I/O request's output buffer.
		}
		break;
	}
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(2, 6, 36)
static int advat24_eeprom_read(struct file *file, char *buf, size_t count, loff_t *ptr)
#else
static ssize_t advat24_eeprom_read(struct file *file, char *buf, size_t count, loff_t *ptr)
#endif
{
	char tmpbuf[2048] = {0};
	ssize_t readlen = 0;
	...
	readlen = at24_read(global_at24, tmpbuf, *ptr, count);
	printk(KERN_ERR "[%s:%d] readlen:%d \n", __func__, __LINE__, readlen);
	//if (readlen != count) {
	//	printk(KERN_ERR "Error: at24_read() \n");
	//	return -EFAULT;
	//}
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(2, 6, 36)
static int advat24_eeprom_write(struct file *file, const char *buf, size_t count, loff_t *ppos)
#else
static ssize_t advat24_eeprom_write(struct file *file, const char *buf, size_t count, loff_t *ppos)
#endif
{
	char tmpbuf[2048] = {0};
	ssize_t writelen = 0;
	...
	writelen = at24_write(global_at24, tmpbuf, *ppos, count);
	if (writelen != count) {
		printk(KERN_ERR "Error: at24_write() \n");
		return -EFAULT;
	}
...
static int advat24_eeprom_open(struct inode *inode, struct file *file)
{
	return 0;
}

static int advat24_eeprom_release(struct inode *inode, struct file *file)
{
	return 0;
}

static struct file_operations advat24_eeprom_fops = {
	owner:		THIS_MODULE,
#if LINUX_VERSION_CODE < KERNEL_VERSION(2, 6, 36)
	ioctl:		advat24_eeprom_ioctl,
#else
	unlocked_ioctl: advat24_eeprom_ioctl,
#endif
	read:		advat24_eeprom_read,
	write:		advat24_eeprom_write,
	open:		advat24_eeprom_open,
	release:	advat24_eeprom_release,
};
#endif // USEFOR_ADVANTECH_AT24C08.

static int __init advat24_init(void)
{
	if (!io_limit) {
...
#ifdef USEFOR_ADVANTECH_AT24C08
	if ((major = register_chrdev(0, "adveeprom", &advat24_eeprom_fops)) < 0) {
	...
	printk("=====================================================\n");
	printk("     Advantech 24c08 eeprom driver V%s [%s]\n", 
			ADVANTECH_EC_EEPROM_VER, ADVANTECH_EC_EEPROM_DATE);
	printk("=====================================================\n");
#endif // USEFOR_ADVANTECH_AT24C08.

	return i2c_add_driver(&advat24_driver);
}
module_init(advat24_init);

static void __exit advat24_exit(void)
{
	i2c_del_driver(&advat24_driver);
...
module_exit(advat24_exit);

MODULE_DESCRIPTION("Drivers for Advantech TPC-71W AT24C08 I2C EEPROM");
MODULE_AUTHOR("Ji.Xu");
MODULE_LICENSE("GPL");
